...
Ideally, we would like to extend the vector space while mantaining the usal notion of real numbers\footnote{Non standard standard analists have no problem in expanding $\mathds{R}$. In these notes, for a change, we insist as $\mathds{R}$ should remain $\mathds{R}$ throughout.} but this is incompatible with the usual notion of elementary extension.
One way out has been proposed by Henson and Iovino: restrict the notion of elementarity to a smaller class of formulas.
We elaborate on this idea generalizing it to arbitrary structures and to a larger class of formulas.
...
The function symbols of $L'$ fall in one of three groups
  \begin{itemize}
    \item[0.] $M^n\to M$ we asume, for simplicity, there are in $L$;
...
Clearly, there are many more terms in $L'$ but we will restrict to these.
...
\begin{definition}\label{def_LL}
  ...
  \begin{itemize}
  \item[i.] formulas as $t(y;z)\in C$, where $C\subseteq R^n$ is compact\footnote{We confuse the relation symbols in $L'$ of sort $R^m$ with their interpretation and write $x\in C$ for $C(x)$} and $t(y;z)$ is a tuple of terms in $\mathds{T}_R\circ\mathds{T}_M$; 
  \item[ii.] all formulas in $L$.
  \end{itemize}
  We require that $\mathds{L}$ is closed under the Boolean connectives $\wedge$, $\vee$, the quantifiers $\forall$, $\exists$ of sort $M$; and the universal quantifier $\forall$ of sort $R$ when restrected to a bounded subset of $R$.
\end{definition}
...
\ceq{\hfill\bigwedge_{\varphi'>\varphi}\varphi'}{\leftrightarrow}{\varphi.}

\bigskip
Finally, we prove $\mathds{L}$\llap{\rotatebox{30}{--}\kern1.5pt}\v{o}\'s Theorem.

\def\ceq#1#2#3{\parbox[t]{18ex}{$\displaystyle #1$}\parbox{5ex}{\hfil $#2$}{$\displaystyle #3$}}

\begin{proposition}\label{thm_Los}
  Assume that the weight of the topology of $R$ is $\le|I|$.
  ...
  {\Leftrightarrow}{}
  \parbox[t]{60ex}{
    for every $\varphi'>\varphi$ there is an 
    $U\ni \alpha$ such that for every 
    $\beta\in U$
    \par\smallskip\hfil
    $\big\{ i\in I\ :\ {\EuScript M}\models\varphi'(\hat ai;\beta)\big\}\in F$}
\end{proposition}
\begin{proof}
  First note that if $\varphi(x;y)$ is as in (ii) of Definition~\ref{def_LL} then the proposition reduces to the classical \L\v{o}\'s Theorem.

  Now, suppose that $\varphi(x;y)$ is as in (i) of Definition~\ref{def_LL}, say it is the formula $t(x;y)\in C$, where $t(x;y)\in \mathds{T}_R\circ\mathds{T}_M$.
  
  Assume ${\EuScript N}\models t(\hat a,\alpha)\in C$.
  Let $\varphi'>\varphi$ be of the from $t(\hat a,\alpha)\in D$ where $D$ is a compact neighborhood of $C$.
  Then there is an open set $O$ such that $C\subseteq O\subseteq D$.
  By continuity there is an open set $U\ni\alpha$ such that $t^{\EuScript N}(\hat a,\beta)\in O\subseteq D$ for every $\beta\in U$.
  By the definition of $F$-limit, $\big\{ i:{\EuScript M}\models t(\hat ai,\beta)\in D\big\}\in F$.
  
  Vice versa, assume ${\EuScript N}\models t(\hat a,\alpha)\notin C$.
  Let $D$ be a compact neighborhood of $C$ such that $t^{\EuScript N}(\hat a,\alpha)\notin D$.
  By continuity, there is an open $U\ni\alpha$ such that ${\EuScript N}\models t(\hat a,\beta)\notin D$ for every $\beta\in U$.
  By Fact~\ref{fact_termini2} and the definition of $F$-limit, $\big\{ i:{\EuScript M}\models t(\hat ai,\beta)\notin D\big\}\in F$ for every $\beta\in U$.

  Induction for the connectives $\vee$ and $\wedge$ is clear.
  To deal with the universal quantifier of sort $R$ we assume inductively that 
 
  \ceq{\hfill {\EuScript N}\models\varphi(\hat a; \alpha, \beta)}
  {\Leftrightarrow}{}
  \parbox[t]{63ex}{
    for every $\varphi'>\varphi$ there are
    $U,V\ni \alpha,\beta$ such that for every 
    $\alpha',\beta'\in U,V$
    \par\smallskip\hfil
    $\big\{ i\in I\ :\ {\EuScript M}\models\varphi'(\hat ai;\alpha',\beta')\big\}\in F$}
    
  and prove that
 
  \ceq{\hfill {\EuScript N}\models\forall y\,\varphi(\hat a; \alpha, y)}
  {\Leftrightarrow}{}
  \parbox[t]{63ex}{
    for every $\varphi'>\varphi$ there is an 
    $U\ni \alpha$ such that for every 
    $\alpha'\in U$
    \par\smallskip\hfil
    $\big\{ i\in I\ :\ {\EuScript M}\models\forall y\,\varphi'(\hat ai;\alpha',y)\big\}\in F$.}
  
  To prove $\Leftarrow$ assume ${\EuScript N}\not\models\varphi(\hat a; \alpha, \beta)$ for some $\beta$.
  By induction hypothesis, there is some $\varphi'>\varphi$
  
  \ceq{ }{ }{}
  \parbox[t]{63ex}{
    for every
    $U,V\ni \alpha,\beta$ there are
    $\alpha',\beta'\in U,V$ such that
    \par\smallskip\hfil
    $\big\{ i\in I\ :\ {\EuScript M}\models\varphi'(\hat ai;\alpha',\beta')\big\}\notin F$.}
  ...
  \parbox[t]{63ex}{
    for every
    $U\ni \alpha$ there is some
    $\alpha'\in U$ such that
    \par\smallskip\hfil
    $\big\{ i\in I\ :\ {\EuScript M}\models\forall y\,\varphi'(\hat ai;\alpha',y)\big\}\notin F$,}

   as required. 

  To prove $\Rightarrow$ assume ${\EuScript N}\models\forall y\,\varphi(\hat ai;\alpha,y)$ and, for a contrdiction, that there is some $\varphi'>\varphi$
  ...
  \parbox[t]{63ex}{
    ...
    $\alpha'\in U$ such that
    \par\smallskip\hfil
    $\big\{ i\in I\ :\ {\EuScript M}\not\models\forall y\,\varphi'(\hat ai;\alpha',y)\big\}\in F$.}
  
  We are using that $F$ is an ultrafilter. 
  Let $\langle U_i:i\in I\rangle$ be a sequence of neighborhoods of $\alpha$ such that for every $A\in F$
  
  \ceq{\hfill\bigcap_{i\in A} U_i }{=}{\{\alpha\}}
  
  Pick $\alpha_j\in U_j$ a $\beta_{j,i}$ such that
  
  \ceq{ }{ }{}
  \parbox[t]{63ex}{\hfil
    $\big\{ i\in I\ :\ {\EuScript M}\not\models\varphi'(\hat ai;\alpha_j,\beta_{i,j})\big\}\in F$.}
  
  Let $\beta=F\mbox{-}\lim_j\big(F\mbox{-}\lim_i\beta_{j,i}\big)$.
  As ${\EuScript N}\models\varphi'(\hat ai;\alpha,\beta)$, from the induction hypothesis we obtain $U,V\ni\alpha,\beta$ such that for every  $\alpha',\beta'\in U,V$
  ...
  We first we prove that

  \ceq{\hfill {\EuScript N}\models\forall y\,\varphi(\hat a,y\,; \alpha)}
  {\Leftrightarrow}
  ...
  Assume for simplicity that $I=\omega$ and that $R$ is second countable.
