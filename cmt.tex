...
We define an equivalence relation $\sim_M$ on the unit ball of any given model $M$ a follows
$$
a\, \sim_Mb\quad\Leftrightarrow\quad\varphi(a)^M=\varphi^M(b) \quad\textrm{for every }\varphi(x)\in L(M)
$$
In ther words, $a\,\sim_Mb$ iff $I_M\cup\{(a,b)\}\subseteq M^2$ is an elementary relation.
If $a'\, \sim_Mb'$ then, as elementary relations are closed under composition, $I_M\cup\big(\{a,a'\}\times\{b,b'\}\big)\subseteq M^2$ is also an elementary relation.
In general, we have that
$$
a_i\, \sim_Mb_i\textrm{ for every }i\in(n]\quad\Leftrightarrow\quad\varphi(a_1,\dots,a_n)^M=\varphi^M(b_1,\dots,b_n) \quad\textrm{for every }\varphi(x)\in L(M)
$$
...
If $R\subseteq M^2$ is elementary, then $RR^{-1}\subseteq(\sim_M)$.
