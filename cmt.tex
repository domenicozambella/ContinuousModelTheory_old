...
\begin{definition}\label{def_0}
  ...
  When suggested by the context, $x\in C$ stands for $C(x)$.
...
In a nutshell, we require that function symbols of the second clade above do not occur under the scope of a negation.
...
\begin{definition}\label{def_LL}
  Formulas in \emph{$\mathds{L}$\/} are constricted inductively from two clades of formulas below
  \begin{itemize}
...
\end{proposition}
\begin{proof}
  First note that if $\varphi(x;y)$ is as in (ii) of Definition~\ref{def_LL} then, if we absorbe $\alpha$ into the language, the proposition reduces to the classical \L\v{o}\'s Theorem.
