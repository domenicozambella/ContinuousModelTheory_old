...
\begin{definition}\label{def_0}
  ...
  We assume that $R$ is endowed with a Hausdorff topology and that there is a relation symbol in $L'$ for every compact $C\subseteq R$. We will write $x\in C$ for $C(x)$.
  
  We require that the function symbols of sort $M^n\times R^m\to R$  are interpreted in functions such that
  \begin{itemize}
    \item[1.] $f^{\EuScript M}:M^n\times R^n\to R$ is bounded, i.e.\@ its range is contained in a compact set;
    \item[2.] $f^{\EuScript M}:M^n\times R^n\to R$ is continuous uniformly in $M$, see Definition~\ref{def_unif_cont}.
 
    % \item[2] $f^{\EuScript M}(a;\mbox{-}):R^m\to R$ is are 
    % uniformly continuous, uniformly in $a\in M^n$. Precisely, we require the following. Fix a compact $C\subseteq R$ containgng the range of $f^{\EuScript M}$, let $U$ and $V$ range over the entourages of the (unique)uniformity, associated to $C$. We require that
    
    % for every $U$ there is a $V$ such that for every $a,b\in M^n$ 
...
As a remedy, we carve out a set of formulas $L\subseteq\mathds{L}\subseteq L'$ such that every model as in Definition~\ref{def_0} has an $\mathds{L}$-elementary extension in the same class that is $\mathds{L}$-saturated.

In a nutshell, formulas in $\mathds{L}$ are required not to contain quantifiers of sort $R$.
Moreover, function symbols in the second group below may only occur only inside compact relations and never under the scope of negation.

Function symbols are divided in two groups according to their sort
  \begin{itemize}
    \item[1.] $M^n\times R^m\to M$
    \item[2.] $M^n\times R^m\to R$
  \end{itemize}
 The ultrapower construction we introduce in the next section is completely classical w.r.t.\@ functions in 1. 
 On the other hand, functions in 2 are \textit{critical\/} and need to be dealt with differently.
...
Clearly, there are many more terms in $L'$ but for simplicity, we will restrict the discussion to these.

\begin{definition}\label{def_1}
  Formulas in \emph{$\mathds{L}$\/} are defined inductively as follows
  \begin{itemize}
  \item[i.] $\mathds{L}$ contains formulas of the form $t(y;z)\in C$,where $C\subseteq R^n$ is compact and $t(y;z)$ is a tuple of terms in $\mathds{T}_R\circ\mathds{T}_M$; 
  \item[ii.] $\mathds{L}$ contains all formulas without quantifiers of sort $R$ and only terms in $\mathds{T}_M$;
  \item[iii.] $\mathds{L}$ is closed under the Boolean connectives $\wedge$, $\vee$, and the quantifiers $\forall$, $\exists$ of sort $M$;
  \item[iv.] ???  it might be feasible to assume some form of closure under the quantifier $\forall$ of sort $R$.
  \end{itemize}
\end{definition}
...
It is convenient to unravel the devinition above into a longer b.
...
We require that each function symbol $f$ of sort $M^n\times R^m\to R$ is a compact $C\subseteq R$ that contains the range of all the functions $f^{{\EuScript M}_i}$. 
(Otherwise, Fact~\ref{fact_termini1} below may fail.)
...
\begin{proof}
  By induction. 
  The fact holds by definition if $t$ is atomic.
  Therefore, assume $\#_0$ and prove 
  ...
  \ceq{\#_2\hfill f^{\EuScript N}\Big(\hat a;t^{\EuScript N}\big(\hat a;\alpha\big)\Big)}{=}{F\mbox{-}\lim_i f^{\EuScript M}\Big(\hat ai;\lambda\Big).} 
  
  where 
  
  \ceq{\hfill\lambda}{=}{F\mbox{-}\lim_i t^{\EuScript M}\big(\hat ai;\alpha\big)}

  Let $\sigma$ the limit in $\#_2$.
  Let $V\subseteq R$ be an open set containing $\sigma$.
  Apply the uniform continuity as in 2 of Proposition~\ref{prop_uc_esplicito}, to obtain $A\in F$ and an open set $U\subseteq R$ containing $\lambda$ such that for every $i\in A$ and every $x\in U$
  
  \ceq{\hfill f^{\EuScript M}\big(\hat ai;x\big)}{\in}{V}.

  After possibly restricting $A$, we can assume that for every $i\in A$ 

  \ceq{\hfill t^{\EuScript M}\big(\hat ai;\alpha\big)}{\in}{U}.

  This sufficies to prove that $\sigma$ is also the limit in $\#1$.
\end{proof}
