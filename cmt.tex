...
There is a relation symbol for every compact subset of $R^n$ and there are no other relations of sort $R^n$.
...
The class $\mathds{H}$ is the immediate generalization of class of  positive bounded formulas of Henson and Iovino.
We introduced the larger class $\mathds{L}$ because it offers some advantages. 
For instance, it is easy to see that $\mathds{L}$ has at least the same expressive power of real valued logic (in fact, way more).
However, we will see that the two classes of formulas are closer than one might expect.
...
\begin{proof}
  ...
  Then $\tilde{\varphi}=(t\in C'\smallsetminus O)$ is as required.
  
  Suppose instead that $\tilde{\varphi}$ is of the form $t\in C'$ for some compact $C'$ disjoint from $C$.
  By the local compactness of $R$, there exist $C''$ and $O$, disjoint neighborhoods of $C$ and $C'$ respectively, where is $C''$ is compact.
...
\begin{definition}\label{def_uniformity}
  We say that a family of models $\{{\EuScript M}_i:i\in I\}$ is uniformly bounded if there is a compact $C\subseteq R$ that contains the range of all the functions $f^{{\EuScript M}_i}$.
\end{definition}
...
The same notation is used with $\mathds{H}$ for $\mathds{L}$.
...
% If the map ${\rm id}_A:M\to N$ is approximatively $\mathds{L}$-elementary, we write \emph{${\EuScript M}\equiv^\mathds{A}_A{\EuScript N}$.}

% \begin{fact}
%   Every $\mathds{L}$-elementary map is approximatively $\mathds{L}$-elementary
% \end{fact}

% \begin{proof}
%   Only the second implication above requires a proof.
...
% \begin{proof}
%   (1$\Rightarrow$2) If ${\EuScript N}\models\exists x\,\neg\varphi(x)$ then ${\EuScript N}\not\models\forall x\,\varphi(x)$ and, by (1), ${\EuScript M}\not\models\forall x\,\varphi(x)$.
%   Then ${\EuScript M}\not\models\varphi(a)$ for some $a\in M$ and ${\EuScript N}\models\neg\varphi(a)$ follows from (1).
...
%   Therefore, for every formula $\varphi(x)$ as in (i) and (ii) of Definition~\ref{def_LL} we have that ${\EuScript M}\models\varphi(a)\Leftrightarrow{\EuScript N}\models\varphi(a)$ for every $a\in M^{|x|}$
...
As in the classical case, a theory $T\subseteq\mathds{L}$ is said to be finitely consistent if every $\varphi$, conjunction of sentences in $T$, is consistent.
We further say that $T$ is \emph{uniformly\/} finitely consistent if the finite consistency is witnessed by uniformly bounded models (see Definition~\ref{def_uniformity}).
...
Below we say \emph{model\/} for $\mathds{L}$-elementary substructure of ${\EuScript U}$.
...
When $A\subseteq U$, we write $S_{\mathds{H},x}(A)$ for the set of types 

\ceq{\hfill\mathds{H}\mbox{-tp}(a/A)}{=}{\big\{\varphi(x)\ :\ \varphi(x)\in\mathds{H}(A)\textrm{ such that }\varphi(a)\big\}}

 as $a$ ranges over the tuples in $U^{|x|}$.
 The following corollary will be strengthen by Corollary~\ref{corol_Lcomplete} below.

 \begin{corollary}\label{corol_Hcomplete2}
   The types $p(x)\in S_{\mathds{H},x}(A)$ are complete.
   That is, for every $\varphi(x)\in\mathds{H}(A)$, either $p(x)\vdash\varphi(x)$ or $p(x)\vdash\neg\varphi(x)$.\hfill\qedsymbol
 \end{corollary}

 \begin{proof}
  Follows from the proposition above and Lemma~\ref{lem_interpolation}.
 \end{proof}

For $p(x)\subseteq\mathds{L}(U)$, we write $p'(x)$ for the type

\ceq{\hfill p'(x)}{=}{\big\{\varphi'(x):\varphi'>\varphi\textrm{ for some }\varphi(x)\in p\big\}.}

Note that, by the proposition above, if $p(x)\in S_{\mathds{H}}(A)$ then $p'(x)\leftrightarrow p(x)$.
Therefore $p'(x)$ is also complete.
...
\begin{corollary}\label{corol_H=L}
  For every $a,b\in U^{|x|}$, if $a\equiv^\mathds{H}b$ then  $a\equiv^\mathds{L}b$.
\end{corollary}
...
\begin{proposition}\label{}
  ...
  Then there is a consistent $\psi(x)\in\mathds{H}(A)$ and some $\psi'>\psi$ such that $\psi'(x)\rightarrow\neg\varphi(x)$.\hfill\qedsymbol
\end{proposition}
...
\begin{proposition}[Tarski-Vaught Test] 
  ...
  \begin{itemize}
    ...
    \noindent\kern-\leftmargin
    \ceq{\hfill{\EuScript U}\models\exists x\,\varphi(x)}{\Rightarrow}
    {\textrm{ for every }\varphi'>\varphi\textrm{ there is an }a\in M\textrm{ such that }{\EuScript U}\models\varphi'(a);}
    \item[3.] for every formula $\varphi(x)\in\mathds{L}(M)$
    
    \noindent\kern-\leftmargin
    \ceq{\hfill{\EuScript U}\models\exists x\,\neg\varphi(x)}{\Rightarrow}
    {\textrm{ there is an }a\in M\textrm{ such that }{\EuScript U}\models\neg\varphi(a).}
    
  \end{itemize}
\end{proposition}

\begin{proof}
  (1$\Rightarrow$2) Assume ${\EuScript U}\models\exists x\,\varphi(x)$ and let $\varphi'>\varphi$ be given.
  ...
  Then ${\EuScript U}\not\models\forall x\,\tilde{\varphi}(x)$ hence, by (1), ${\EuScript M}\not\models\forall x\,\varphi(x)$.
  Then ${\EuScript M}\models\neg\tilde{\varphi}(a)$ for some $a\in M$. Hence ${\EuScript M}\models\varphi'(a)$ and ${\EuScript U}\models\varphi'(a)$ follows from (1).

  (2$\Rightarrow$3)
  Assume (2) and let $\varphi(x)\in\mathds{L}(M)$ be such that ${\EuScript U}\models\exists x\,\neg\varphi(x)$.
  Then, by Corollary~\ref{corol_Hcomplete3}, there are a consistent $\psi(x)\in\mathds{H}(M)$ and some $\psi'>\psi$ such that $\psi'(x)\rightarrow\neg\varphi(x)$.
  Then (3) follows.
  ...
  Therefore ${\EuScript M}\models\varphi(a)\Leftrightarrow{\EuScript U}\models\varphi(a)$ for every $a\in M^{|x|}$ and for every formula $\varphi(x)$ as in (i) and (ii) of Definition~\ref{def_LL}.

  Now, assume inductively
  
  \ceq{\hfill{\EuScript M}\models\varphi(a,b)}{\Rightarrow}{{\EuScript U}\models\varphi(a,b)}

  Using (3) and the induction hypothesis we prove by contrapposition that

  \ceq{\hfill{\EuScript M}\models\forall y\,\varphi(a,y)}{\Rightarrow}{{\EuScript U}\models\forall y\,\varphi(a,y)}.

  Indeed,

  \ceq{\hfill{\EuScript U}\not\models\forall y\,\varphi(a,y)}
  {\Rightarrow}{{\EuScript U}\models\exists y\,\neg\varphi(a,y)}
