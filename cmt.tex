...
We write ${\rm Aut}(M)$ for the set of maximal, total and surjective, elementary relations $R\subseteq M^2$.
The choice of the symbol Aut is motivated by the lemma above.
In fact any such relation $R$ induces a unique automorphism on the (properly defined) quotient structure $M/{\sim_M}$.
...
Finally, we write \emph{$M\preceq N$\/} if $M\equiv_M N$ and ${\EuScript M}$ is a substructure of $\bar N$ in the classical sense.
...
\begin{proposition}[Tarski-Vaught test]\label{prop_Tarski-Vaught} Let $M\subseteq N$ and assume that ${\EuScript M}$ is a substructure of $\bar N$.
...
We define a $3$-sorted structure $\langle{\EuScript A},{\EuScript R},{\EuScript M}\rangle$.

Fix a nonempty set $\Omega$.

The domain of the first sort consists of a Boolean algebra ${\EuScript A}\subseteq{\EuScript P}(\Omega)$.
The language contains functions for the Boolean operations on ${\EuScript A}$.
% The unit ball is the whole of ${\EuScript A}$.
...
% We make use of $\Omega$ and  $\sim_{\EuScript A}$ in the definitions of $\langle{\EuScript A},{\EuScript R},{\EuScript M}\rangle$ but they will not directly appear in the structure.

% We now define ${\EuScript R}$ and ${\EuScript M}$.
...
The domain of the second sort ${\EuScript R}$ contains the functions $f:\Omega\to{\mathds R}$ that are linear combinations of indicator functions of sets in ${\EuScript A}$.
In the language we include the functions that make ${\EuScript R}$ an ${\mathds R}$-algebra (sum and and multiplications are defined pointwise).
...
The domain of the third sort, ${\EuScript M}$, contains signed measures $\mu:{\EuScript A}\to{\mathds R}$ that are linear combinations of measures of the form $\delta_a$, for $a\in\Omega$.
These are defined as follows: $\delta_a(A)$ is $1$ if $a\in A$ and $0$ otherwise.
The language is that of lattice vector spaces.
% The unit ball, $M$, contains the measures in ${\EuScript M}$ that are $\le1$ in absolute value.

There is a function of sort ${\EuScript R}\times{\EuScript M}\to{\EuScript M}$ that maps $(f,\mu)$ to the measure $f\mu$ obtained multiplying in the natural way $f$ and $\mu$.

Finally, there is a predicate $I$ of sort ${\EuScript A}\times{\EuScript M}$ such that $I(A,\mu)=\mu(A)$.

The unit ball $\langle{\EuScript A},{\EuScript R},{\EuScript M}\rangle$ is $\langle{\EuScript A},R,M\rangle$ where $R$ and $M$ are the functions, respectively measures  that are $\le1$ in absolute value.

\hfil ***


The following finite version of the Radon-Nikodym theorem is completely trivial.
For simplicity we state it for measures.
As the Hahn decomposition theorem holds (trivially) in ${\EuScript M}$, this is no loss of generality.

\begin{fact}\label{thm_fRN}
  [Discrete Radon-Nikodym]
  Let $\mu,\nu\in M$, where $M$ is the unit ball of ${\EuScript M}$, be non negative.
  For every $\varepsilon > 0$ there are $E\in{\EuScript A}$ and $f\in R$, the unit ball of ${\EuScript R}$, such that
  \begin{itemize}
    \item[1.] $I(\neg E,|\nu-\varepsilon^{-1}f\mu|)\ =\ 0$
    \item[2.] $|I(E, \mu)|\le\varepsilon$.
  \end{itemize}
\end{fact}
...
\begin{fact}\label{thm_fRN2}
  ...
  Let $\mu,\nu\in M$, where $M$ is the unit ball of ${\EuScript M}$.
  ...
  \begin{itemize}
    ...
    \item[1.] $I(E_i\smallsetminus E_{i+1},\ |\nu-2^{-(i+1)}f\mu|)\ =\ 0$
...
\begin{proof}
  ...
  \begin{itemize}
    \item[1$_i$.] $I(X,|\nu-2^{-(i+1)}f_{i+1}\mu|)\ =\ 0$\quad for every $X\subseteq E_i\smallsetminus E_{i+1}$;
    \item[2$_i$.] $|I(X, \mu)|\le2^{-(i+1)}$\quad for every $X\subseteq E_{i+1}$;
    \item[3$_i$.] $f_{i+1}[E_i\smallsetminus E_{i+1}]=\{0\}$.
  \end{itemize}
  Note that the last requirement is immediate to satisfy.
...
%   Let $\mu,\nu\in M$ be such that $\nu\ll\mu$.
%   For every $\varepsilon > 0$ there exist an exceptional set $E\in{\EuScript A}$ and an $f\in S$, the unit ball of ${\EuScript R}$, such that
%   \begin{itemize}
...
We now consider a saturated extension $\langle{}^{*\!\!\!}{\EuScript A},{}^{*\!}{\EuScript R},{}^{*\!}{\EuScript M}\rangle$.
The following fact is a direct consequence of saturation.

\begin{fact}
  Let $\mu$ be a bounded finitely additive measure on ${\EuScript A}$, not necessarily in ${\EuScript M}$.
...
In fact, we can also control the measure of ${}^{*\!\!}\mu(A)$ for any $A\in{\EuScript A}$.

\def\ceq#1#2#3{\parbox[t]{25ex}{$\displaystyle #1$}\parbox{5ex}{\hfil $#2$}{$\displaystyle #3$}}

\begin{fact}
  Let $\mu$ be a bounded measure on ${\EuScript A}$, not necessarily in ${\EuScript M}$.
  Let $A\in{}^{*\!\!\!}{\EuScript A}$.
  Define 
  ...
  Then for every $m_{\rm i\,}\le r\le m_{\rm e}$ there is a measure ${}^{*\!\!}\mu\in{}^{*\!}{\EuScript M}$ such that ${}^{*\!\!}\mu_{\restriction{\EuScript A}}=\mu_{\restriction{\EuScript A}}$ and  ${}^{*\!\!}\mu(A)=r$.
\end{fact}
...
For $i>m$ let $\nu(X_i)=\mu(X_i)$.
Note that $\mu(X_1\cup\dots\cup X_m)\ge m_{\rm e}-m_{\rm i}$.
Therefore with a suitable $\varepsilon$, we can obtain $\nu(A)=r$.
\end{proof}

If ${\EuScript A}$ is a $\sigma$-algebra and $\mu$ is a $\sigma$-additive measure then the suppremum and the infimum in the fact above is attained.
We will use the following.

\begin{corollary}
  If ${\EuScript A}$ is a $\sigma$-algebra and $\mu$ is a bounded $\sigma$-additive measure then for every $A\in{}^{*\!\!\!}{\EuScript A}$ there are $A_{\rm i},A_{\rm e}\in{\EuScript A}$ such that $A_{\rm i}\subseteq A\subseteq A_{\rm e}$ and a measures ${}^{*\!\!}\mu$ such that 
  ${}^{*\!\!}\mu_{\restriction{\EuScript A}}=\mu_{\restriction{\EuScript A}}$ and ${}^{*\!\!}\mu_{\rm e}(A)=\mu(A_{\rm e})$.  
  A similar claim holds for $A_{\rm i}$.
...
\begin{theorem}[Radon-Nikodym]
  Assume ${\EuScript A}$ is a $\sigma$-algebra and let $\nu$ and $\mu$ be bounded $\sigma$-additive measures.
  Then there is a random variable $f$ such that 
  $$
