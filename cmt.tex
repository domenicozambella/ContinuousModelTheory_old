...
\begin{definition}\label{def_0}
  As $M$ ranges over the $L$-structures, we consider the class of $L'$-structures of the form ${\EuScript M}=\langle  M,R\rangle$, where $R$ is some fixed structure.
  We are given a Hausdorff topology on $R$ and assume that there is a relation symbol in $L'$ for every subset of $R^n$ that is compact in the product topology.
  We require that the function symbols of sort $M^n\times R^m\to R$  are interpreted in functions such that
  \begin{itemize}
    \item[1.] $f^{\EuScript M}(\mbox{-};\alpha):M^n\to R$ is bounded (i.e.\@ its range is contained in a compact set) for every $\alpha\in R^m$.
 
    \item[2??] $f^{\EuScript M}(a;\mbox{-}):R^m\to R$ is continuous uniformly in $a\in M^n$, that is (?),
    
    \hfil$\displaystyle\bigcap_{a\in M^n}f^{\EuScript M}(a;\mbox{-})^{-1}(O)$ is open for every open $O\subseteq R$.
...
There may be no saturated structure among those described in Definition~\ref{def_0}.
As a remedy, we restrict the notion of saturation to formulas in a set $\mathds{L}$ that we define below.
In a nutshell, formulas in $\mathds{L}$ do not contain existential quantifiers of sort $R$.
Moreover, we require that a few \textit{critical\/} function symbols only occour only inside compact relations and never under the scope of negation.

Intuitively, the function symbols are divided in two groups according to their sort
  \begin{itemize}
    \item[1.] $M^n\times R^m\to M$
    \item[2.] $M^n\times R^m\to R$
  \end{itemize}
The ultrapower construction we introduce in the next section is completely classical w.r.t.\@ functionsw as in 1. On the other hand, the functions as in 2 are are \textit{critical\/} and need to be dealt diffrently.

Below, \emph{$\mathds{T}$\/} denotes the set of the terms of sort  $M^n\times R^m\to R$. 
Definition~\ref{def_0} ensures that for all $t(x;y)\in\mathds{T}$ the range of the functions $t^{\EuScript M}(\mbox{-},\alpha):M^n\to R$ is bounded.

\begin{definition}\label{def_1}
  Formulas in \emph{$\mathds{L}$\/} are defined inductively as follows
  \begin{itemize}
  \item[i.] $\mathds{L}$ contains all formulas without quantifiers of sort $R$ and functions of sort $M^n\times R^m\to R$; 
  \item[ii.] $\mathds{L}$ contains formulas of the form $t(y;z)\in C$ where $C\subseteq R^n$ is compact and $t(y;z)\in\mathds{T}^n$;
  \item[iii.] $\mathds{L}$ is closed under the Boolean connectives $\wedge$, $\vee$, and the quantifiers $\forall$, $\exists$ of sort $M$;
  \item[iv.] $\mathds{L}$ is closed under the quantifier $\forall$ of sort $R$.
  \end{itemize}
\end{definition}
...
If $f:I\to R^m$ and $\lambda\in R^m$ we write

\hfil$\displaystyle F\mbox{-}\lim_{i}f(i)=\lambda$

if $f^{-1}[A]\in F$ for every $A\subseteq R^m$ that is a neighborhood of $\lambda$.
Such a $\lambda$ is always unique (by the property of filters).
When $F$ is an ultrafilter, and ${\rm range} f$ is bounded (i.e. it is contained in a compact set), the limit always exists.

Below we introduce a suitable notion of ultrapower.
To keep notation tidy, we make two simplifications: (1) we only consider ultraproducts; (2) we do not consider formulas containing equality, so we need not take any quotient.
...
\begin{definition}\label{def_ultraproduct}
  ...
  \begin{itemize}
    ...
    \item[4.] If $f$ is a function of sort $ M^n\times R^m\to R$ then 
    $$
...
\begin{fact}
  The structure ${\EuScript N}$ satisfies the conditions of Definition~\ref{def_0}. (Didin't check condition 2.)
\end{fact}

The following fact is easily proved by induction on the syntax.

\begin{fact}\label{fact_easy_induz}
  If only function symbols of sort $M^n\times R^m\to M$ occurs in  $t(y;z)$ then 
  
  \ceq{\hfill t^{\EuScript N}\big(\hat a;\alpha\big)}{=}{\langle t^{\EuScript M}\big(\hat ai;\alpha\big): i\in I\rangle.}\hfill\qedsymbol
\end{fact}

If true, the proof of the following fact is tedious.

\begin{fact}
  For every term $t(y;z)$ of sort $M^n\times R^m\to R$  
...
\begin{proof}
  ...
  Assume the claim is true for $t_M(y,z)$ a tuple of terms with values in $M$.
  Write $\lambda_\alpha$ for $t^{\EuScript N}_M\big(\hat a;\alpha\big)$.
  Let $t_R(y;z)$ be a tuple of terms with values in $R$.
  Let $f$ be a function of sort $M^n\times R^m\to R$.
  We need to prove that

  \ceq{\hfill f^{\EuScript N}\Big(t^{\EuScript N}_M\big(\hat a;\alpha\big);\lambda_\alpha\Big)}{=}{F\mbox{-}\lim_i f\Big(t^{\EuScript M}_M\big(\hat ai;\alpha\big);t^{\EuScript M}_R\big(\hat ai;\alpha\big)\Big)}
...
For $\varphi(x),\varphi'(x)\in\mathds{L}(A)$ we say that $\varphi'>\varphi$ if $\varphi'$ can be obtained replacing each atomic formulas $t\in C$ occurring in $\varphi$ by $t\in D$ for some closed neighborhood of $C$.
Note that, if such atomic formulas do not occur in $\varphi$, then $\varphi>\varphi$.
...
\end{proposition}
\begin{proof}
  First note that if $\varphi(x;y)$ is as in (i) of Definition~\ref{def_1} then, after absorbing $\alpha$ into the language, the proposition reduces to the classical \L\v{o}\'s Theorem.

  Now, suppose that $\varphi(x;y)$ is as in (ii) of Definition~\ref{def_1}, say it is the formula $t(x;y)\in C$, where $t(x;y)$ is a tuple of terms of sort $M^{|x|}\times R^{|y|}\to R$.
  
  Assume ${\EuScript N}\models t(\hat a,\alpha)\in C$.
  If $D$ a compact neighborhood of $C$ then $D$ is also a neighborhood of $t^{\EuScript N}(\hat a,\alpha)$.
  ...
  We can assume to work inside a compact set containing $C$ and $t(\hat a,\alpha)$. By regularity, $t^{\EuScript N}(\hat a,\alpha)\notin D$ for $D$ a compact neigboorhood of $C$. 
  By the definition of $F$-limit, $\big\{ i:{\EuScript M}\models t(\hat ai,\alpha)\notin D\big\}\in F$.
  Then $\big\{ i:{\EuScript M}\models t(\hat ai,\alpha)\in D\big\}\notin F$.

  As for induction, only the existential quantifier require some work.
  ...
  \ceq{\hfill {\EuScript N}\models\exists y\,\varphi(\hat a,y\,; \alpha)}
  {\Leftrightarrow}
...
\begin{definition}\label{def_uniformity}
  ...
  Let $t(x)\in\mathds{T}(M)^{<\omega}$ be a finte tuple of terms, say $t(x)=t_1(x),\dots,t_n(x)$.
  We define a pseudometric ${\rm d}_t(\mbox{-},\mbox{-})$ on $M$ as follows

  \ceq{\hfill\emph{${\rm d}_t(a,b)$}}{=}{\max_{i=1,\dots,n}\big\{|t_i^{\EuScript M}(a)-t_i^{\EuScript M}(b)|\big\}}
