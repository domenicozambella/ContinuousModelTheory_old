...
Our choice of $\mathds{L}$ may not be the most general.
...
Here we also introduce the class $\mathds{L}$ because it offers some advantages. 
For instance, it is easy to see (cf.\@ the example below) that $\mathds{L}$ has at least the same expressive power as real valued logic, and in fact way more.
However, we will see that the two classes of formulas, $\mathds{H}$ and $\mathds{L}$, are closer than one might expect.
...
The definitions \emph{$\mathds{H}$-elementary\/} map/embedding/substructure are similar.
