...
Different people have searched different paths.
\begin{itemize}
...
\end{fact}
\begin{proof}
  When a function symbol the fact holds by definition.
  For all other terms, the fact follows from Fact~\ref{fact_terms_st} 
\end{proof}
...
\begin{proof}
  ...
  The general case is easily derived from this special case. 
...
\begin{proof}
  The first claim is immediate by saturation.
...
For $a,b\in U$ we write \emph{$a\sim b$\/} if $\forall y\, [t(a,y)=t(b,y)]$ for every parameter free term $t(x,y)$ of sort $M^{1+|y|}\to R$. 
