...
We define an equivalence relation $(\sim_{M\!})$ on the unit ball of a given model $M$ as follows

\ceq{\hfill a\, \sim_Mb}{\Leftrightarrow}{\varphi(a)^M=\varphi^M(b)}\ \ for every $\varphi(x)\in L(M)$.
...
\ceq{\hfill a_i\, \sim_Mb_i\textrm{ for every }i\in(n]}{\Leftrightarrow}{\varphi(a)^M=\varphi^M(b)}\ \ for every $\varphi(z)\in L(M)$.
...
% If $R\subseteq M\times N$ we write $\bar R$ for the relation $(\sim_{M\!})\mathrel{R}(\sim_{N\!})$.
% Clearly, $R\subseteq\bar R$ and, if $R$ is an elementary relation, then so is $\bar R$.

% If $R\subseteq M\times N$ is elementary, then $RR^{-1}\subseteq(\sim_{M\!})$.
