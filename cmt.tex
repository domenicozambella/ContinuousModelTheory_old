...
\begin{example}\label{ex_Rvlogic}
  ...
  \ceq{\hfill\forall x\ \big[t(x)\le\tau\big]}{\wedge}{\forall\varepsilon>0\ \exists x\ \big[\tau\le t(x)+ \varepsilon\big].}\hfill\qedsymbol
\end{example}
...
\ceq{\hfill\alpha\sim\beta}{\Leftrightarrow}{S\models\alpha\in C\leftrightarrow\beta\in C}\quad for every compact $C\subseteq R$.
...
\ceq{\hfill\bigwedge_{i=1}^n\alpha_i\sim\beta_i}{\Leftrightarrow}{S\models\alpha\in C\leftrightarrow\beta\in C}\quad for every compact $C\subseteq R^n$.
...
\ceq{\hfill{}^*{\EuScript N}\models\varphi(a\,;\alpha)}{\Rightarrow}{{\EuScript N}\models\varphi\big(a\,;{\rm st}(\alpha)\big)}
\end{lemma}
...
The definitions of \emph{$\mathds{H}$-elementary\/} map/embedding/substructure are similar.
Note that any total and surjective $\mathds{H}$-elementary map is immediately an ${\EuScript L}$-isomorphism.
...
As usual, we define a topology on $U^{|x|}$ which we call the $\mathds{L}(A)$-topology.
The closed sets of this topology are the sets defined by the types $p(x)\subseteq\mathds{L}(A)$.
This is a compact topology.

The  $\mathds{L}(A)$-topology on $U^{|x|}\times R^{|y|}$ is the product of the $\mathds{L}(A)$-topology on $U^{|x|}$ and the topology on $R^{|y|}$.

\begin{lemma}
  Let $S\subseteq R$ be a compact set.
  For every $\varphi(x\,;y)\in\mathds{L}(A)$ the set $\varphi(U\,;S)$ is compact.
\end{lemma}

\begin{proof}
  We can assume that $S$, or suitable power of it, contains all compact sets that occur in $\varphi(x\,;y)$ as predicate or as bounds of quantifiers.
