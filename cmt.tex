...
\begin{definition}\label{def_1}
  ...
  \end{itemize}
  \item[2.] There is a symbol for every (total) uniformly continuous functions $\mathds{R}^n\to\mathds{R}$.
...
\begin{definition}
  We write \emph{$\mathds{T}(A)$\/} for the set of terms of sort $\breve M^n\times\mathds{R}^m\to\mathds{R}$ with parameters in some $A\subseteq \breve M$.
  Up to equivalence, these terms have the form $f(t(x),y)$, where $t(x)$ is a tuple of terms of sort $\breve M^{|x|}\to\mathds{R}$, and $f$ is a function $\mathds{R}^{|t|+|y|}\to\mathds{R}$.
\end{definition}
...
\begin{example}[Metric spaces of finite diameter]
  Given a metric space $M,{\rm d}$ of finite diameter we define a structure ${\EuScript M}=\langle \breve M,M,\mathds{R}\rangle$ as follows.
  Set $\breve M=M$ and let ${\rm id}:\breve M\to M$ be the identical isomorphism.
...
%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%
\section{\u Elementary relations}

\def\ceq#1#2#3{\parbox[t]{28ex}{$\displaystyle #1$}\parbox{6ex}{\hfil $#2$}{$\displaystyle #3$}}

Let ${\EuScript M}$ and ${\EuScript N}$ be models.
We say that $R\subseteq\breve M\times\breve N$ is an \emph{\u elementary relation\/} between ${\EuScript M}$ and ${\EuScript N}$ if for every $\varphi(x)\in\mathds{L}$

\ceq{\hfill{\EuScript M}\models\varphi(a)}{\Leftrightarrow}{{\EuScript N}\models\varphi(b)} \hfill for every $a$ and $b$ such that $aRb$.

Recall that, when $a=a_1,\dots,a_n$ and $b=b_1,\dots,b_n$ are tuples, $aRb$ stands for $a_iRb_i$ for $i=1,\dots,n$.

We define an equivalence relation \emph{$(\sim_{\EuScript M})$\/} on $\breve M$ as follows

\ceq{1.\hfill a\,\sim_{\EuScript M\!}b}{\Leftrightarrow}{t^{\EuScript M}(a)=t^{\EuScript M}(b)}\ \ for every $t(x)\in\mathds{T}(\breve M)$,

where $|x|=|a|=|b|=1$.

\begin{lemma}
  The relation $(\sim_{\EuScript M\!})\subseteq \breve M^2$ is an \u elementary relation.
  Among these, it is maximal, i.e.\@ no elementary relation contains $(\sim_{\EuScript M\!})$ properly.
\end{lemma}
...
\begin{proof}
  ...
  Let $\Delta$ denote the diagonal relation on $\breve M$.
  Note that $a_i\sim_{\EuScript M\!}b_i$ is equivalent to saying that $\Delta\cup\{(a_i,b_i)\}$ is an \u elementary relation.
  As \u elementary relations are closed under composition $\Delta\cup\big\{(a_1,b_1),\dots,(a_n,b_n)\big\}$ is \u elementary.
...
\begin{lemma}
  Let $R\subseteq \breve M\times\breve N$ be total and surjective elementary relation.
  Then there is a unique maximal elementary relation containing $R$.
  This maximal elementary relation is equal to both $(\sim_{\EuScript M\!})\,R$ and $R\,(\sim_{\EuScript N\!})$.
\end{lemma}
\begin{proof}
  It is immediate to verify that $(\sim_{\EuScript M\!})\,R$ is an elementary relation containing $R$.
...
\begin{fact}
  For every term $t(x)$ of sort $\breve M^n\to\mathds{R}$  
...
\begin{proof}
  ...
  where $t$ is a tuple of terms of sort  $\breve M^{|x|}\to\mathds{R}$ and $f$ is a uniformly continuous function $\mathds{R}^{|t|+|y|}\to\mathds{R}$. 
