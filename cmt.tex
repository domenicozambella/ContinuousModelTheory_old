...
\begin{definition}\label{def_0}
  ...
  The new symbols are function symbols of sort $M^n\to R$. 
\end{definition}
...
For later reference we remark the following which is an immediate consequence of the restrictions imposed on the sorts of the function symbols.
...
\begin{proof}
  Assume ${\rm st}(\alpha)\notin C$.
  By normality there is a $D$, compact neighborhood of ${\rm st}(\alpha)$ disjoint from $C$.
...
\begin{proposition}[$\mathds{L}$-compactness]\label{prop_compattezza}
  Let $T\subseteq\mathds{L}$ be finitely consistent.
  Then $T$ is consistent.
\end{proposition}

\begin{proof}
  By the compactness theorem ${}^*{\EuScript M}\models T$ for some ${\EuScript L}$-structure ${}^*{\EuScript M}=\langle M,{}^*\!R\rangle$.
...
The \emph{$\mathds{L}(A)$-topology\/} on $U^{|x|}\times R^{|y|}$ is the product of the $\mathds{L}(A)$-topology on $U^{|x|}$ and the topology on $R^{|y|}$.
...
%
  ...
  \ceq{\hfill\bigwedge_{\varphi'>\varphi}\varphi'(x,z\,;y)}
  {\rightarrow}
  {\varphi(x,z\,;y)}
...
Replace $x,y$ with parameters, say $a,\alpha$ and assume the antecedent.
But $\{\exists z\,\varphi'(a,z\,;\alpha):\varphi'>\varphi\}$ implies the finite concistency of the type $\{\varphi'(a,z\,;\alpha):\varphi'>\varphi\}$.
...
Therefore $\exists z\,\{\varphi'(a,z\,;\alpha):\varphi'>\varphi\}$ follows by saturation.
%
...
%  Similarly we define \emph{$S_\mathds{L}(A)$\/} and \emph{$S_{\mathds{L},x}(A)$.}
 ...
 \begin{remark}
  ...
  % We write \emph{$p'(x)$\/} for the type
...
Clearly, an automorphism is ${\EuScript L}$-elementary.
...
\begin{corollary}\label{corol_H=L}
  For every $a,b\in U^{|x|}$, if $a\equiv^\mathds{H}b$ then  $a\equiv^{\EuScript L}b$.
\end{corollary}

As promized, we can now strngthen Corollary~\ref{corol_Hcomplete2}.
...
The next two propositions show that formulas in $\mathds{L}(A)$ are very well approximated by formulas in $\mathds{H}(A)$.
...
\begin{proof}
  ...
  We also have that  ${\EuScript M}$ is an ${\EuScript L}$-submodel of ${\EuScript U}$.
