...
\begin{definition}\label{def_0}
  ...
  We are given, for each $n$,  a set of definable subsets of $R^n$ that form a base of closed sets of a compact Hausdorff topology.

  (?) We require that the terms of sort $M^n\times R^m\to R$  are interpreted in functions $t^{\EuScript M}(a,\mbox{-}):R^m\to R$ that are continuous uniformly in $a\in M^n$. 
\end{definition}

Caveat: occasionally, we work with totopogical spaces that are not ${\rm T}_0$.
When we say that such a space is Hausdorff, we mean that its Kolmogorof quotient is Hausdorff.
  
There may be no saturated structure among those described in Definition~\ref{def_0}.
As a remedy, we restrict the notion of saturation to formulas in $\mathds{L}$, as defined below.
In a nutshell, formulas in $\mathds{L}$ do not contain existential quantifiers of sort $R$.
Moreover, there are some \textit{critical\/} function symbols that may not occur under the scope of negation.
Intuitively, the function symbols are divided in three classes according to their sort
  ...
  \end{itemize}
Functions in the latter group are critical and will be dealt with caution.

Below, \emph{$\mathds{T}$\/} denotes the set of the terms of sort  $M^n\times R^m\to R$.

\begin{definition}\label{def_1}
  The set \emph{$\mathds{L}$\/} is a subset of $L$ that is defined inductively as follows
  \begin{itemize}
  \item[i.] it contains all formulas without quantifiers of sort $R$ and function of sort $M^{n+1}\times R^m\to R$; 
  \item[ii.] it contains the formulas $t(y;z)\in C$ where $C\subseteq R^n$ is a basic closed set and $t(y;z)\in\mathds{T}^n$;
  \item[iii.] it is closed under the Boolean connectives $\wedge$, $\vee$, and the quantifiers $\forall$, $\exists$ of sort $M$;
  \item[iv.] it is closed under the quantifier $\forall$ of sort $R$.
  \end{itemize}
\end{definition}
...
We recall some standard definitions about $F$-limits.
...
Such a $\lambda$ is unique and, when $F$ is an ultrafilter, it always exists (because $R^n$ is compact).

Below we introduce ultrapowers of a suitable form.
To keep the notation as clean as possible we make two simplifications: (1) we discuss only ultraproducts; (2) we do not consider formulas containing equality among elements of $M$, so we need not take any quotient.
...
\begin{proof}
  ...
  If $D$ a closed neighborhood of $C$ then $D$ is also a neighborhood of $t^{\EuScript N}(\hat a,\alpha)$.
  ...
  By normality, $t^{\EuScript N}(\hat a,\alpha)\notin D$ for $D$ a closed neigboorhood of $C$. 
  By the definition of $F$-limit, $\big\{ i:{\EuScript M}\models t(\hat ai,\alpha)\notin D\big\}\in F$.
  Then $\big\{ i:{\EuScript M}\models t(\hat ai,\alpha)\in D\big\}\notin F$.

  As for induction, only the existential quantifier require some work. Assume for simplicity that $I=\omega$.
...
As $\varphi_j(x,y\,;\alpha)\rightarrow\varphi_{j'}(x,y\,;\alpha)$ for $j>j'$, the diagonal sequence $\hat d=\langle \hat b_ii :i\in I\rangle$ satisfies ${\EuScript N}\models\varphi(\hat a,\hat d\,; \alpha)$.
\end{proof}
