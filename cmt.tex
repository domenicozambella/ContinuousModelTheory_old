...
\begin{definition}\label{def_ultraproduct}
  We define a structure \emph{${\EuScript N}=\langle  N,\bar R\rangle$\/} that we call the \emph{ultrapower\/} of ${\EuScript M}$.
  \begin{itemize}
    \item[1.] $N$ is the set of sequences $\hat a: I\to M$.
    \item[2.] $\bar R$ is the set of bounded sequences $\hat\alpha: I\to R$.
    \item[3.] If $f$ is a function symbols then $f^{\EuScript N}(\hat a;\hat\alpha)$ is the sequence 
    $\langle f^{\EuScript M}\big(\hat ai;\hat\alpha i\big):i\in I\rangle$.
    \item[4.] if $r$ is a relation symbol of sort $M^n\times R^m$ then
    $$
    {\EuScript N}\models r(\hat a; \hat\alpha)
    \Leftrightarrow
    \big\{ i\in I\ :\ {\EuScript M}\models r(\hat ai;F\mbox{-}\lim_i\hat\alpha i)\big\}\in F.
    $$
  \end{itemize}
\end{definition}

We define an equivalence relation on ${\EuScript N}$.
\begin{itemize}
  \item[1.] $\hat a\sim_F\hat b$, where $\hat a,\hat b\in N$, if $\{i:\hat ai=\hat bi\}\in F$
  \item[2.] $\hat\alpha\sim_F\hat\beta$, where $\hat\alpha,\hat\beta\in\bar R$, if  $F\mbox{-}\lim_i\hat\alpha i=F\mbox{-}\lim_i\hat\beta i$
\end{itemize}

\begin{fact}\label{fact_congruence}
  The relation ${\sim_F}$ is a congruence.
\end{fact}

\begin{proof}
  If $f$ is a function symbol of sort $M^n\times R^m\to R$ then we need to show that

  \ceq{\hfill f^{\EuScript N}(\hat a\,;\hat\alpha)}{\sim_F}{f^{\EuScript N}(\hat b\,;\hat\beta)}

  \ceq{\hfill F\mbox{-}\lim_i f^{\EuScript M}(\hat ai,\hat\alpha i)}{=}{F\mbox{-}\lim_i f^{\EuScript M}(\hat bi,\hat\beta i)}

  If $f$ is a function symbol of sort $M^n\times R^m\to M$ the claim is clear.
\end{proof}
...
\begin{proof}
  ...
  \ceq{\#_1\hfill f^{\EuScript N}\Big(\hat a;t^{\EuScript N}\big(\hat a;\hat\alpha\big)\Big)}{=}{F\mbox{-}\lim_i f^{\EuScript M}\Big(\hat ai;t^{\EuScript M}\big(\hat ai;\hat\alpha\big)\Big).}
