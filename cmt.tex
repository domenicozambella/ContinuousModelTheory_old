...
The following definition extends that of classical first-order logic with cluses 3 and 4.
...
\item[4.] a distingished unary relation symbol $u$.
\end{itemize}
\end{definition}
...
The \emph{propositional\/}  (or \emph{Riesz\/}) connectives are those in $\big\{+,\,\cdot\,,\wedge\,\big\}\cup{\mathds R}$, where $+$, $\,\cdot\,$, and $\wedge$ are binary connectives, the elements of ${\mathds R}$ are logical constants (i.e.\@ a zero-ary connectives).
...
\item[ii.] $\alpha$ is a formula for every $\alpha\in{\mathds R}$;
...
\begin{minipage}[t]{.4\textwidth}
\begin{itemize}
\item[iv.] $\varphi\cdot\psi$ is a formula;
\item[v.] $\varphi\wedge\psi$ is a formula;
...
\begin{remark}
  It is easy to see that conditions and co-conditions are cosed under logical disjunction and logical conjunction.
...
\ceq{\hfill \varphi^M(a)}{=}{\varphi^N(ka)} \hfill for every $\varphi(x)\in L$ and every $a\in({\rm dom} k)^{|x|}$. 


An \emph{elementary embedding\/} is an total elementary map.
Note that we are slightly abusing the word embedding, in fact elementary maps need not be injective.

We write \emph{$M\equiv N$\/} when $\varnothing:M\to N$ is an elementary map.
In word we say that $M$ and $N$ are \emph{elementarily equivalent.}
We write \emph{$M\equiv_A N$\/} if ${\rm id}_A:M\to N$ is an elementary map.
...
\begin{proof}
  ...
  \ceq{\hfill M\models\varphi(a)<r}{\Leftrightarrow}{N\models\varphi(a)<r} \hfill for every $a\in M^{|x|}$
  ...
  \ceq{\hfill N\,\models\,\smash{\bigwedge_x}\varphi(x,a)<r}{\Leftrightarrow}{} there is a $b\in(uN)$ such that $N\models\varphi(b,a)<r$

  \ceq{}{\Leftrightarrow}{} there is a $b\in N^{|y|}$ such that $N\models (ub<1$ and $\varphi(b,a)<r)$

  \ceq{a.}{\Leftrightarrow}{} there is a $b\in M^{|x|}$ such that $N\models (ub<1$ and $\varphi(b,a)<r)$

  \ceq{b.}{\Leftrightarrow}{M\,\models\,\bigwedge_x\varphi(x)<r.}
...
\ceq{\textrm{Assume\ that\ for\ some\ }\hat b\in(uN)\hfill\varphi^N\big(\hat a,\hat b\big)}{<}{r}.
...
\ceq{\textrm{Hence}\hfill \inf\Big\{\varphi^{M_i}\big(\hat ai,b\big):\,b\in(uM_i)\Big\}}{<}{r}\quad for every $i\in u$.

\ceq{\textrm{Finally,}\hfill\lim_{i\uparrow F}\ \inf\Big\{\varphi^{M_i}\big(\hat ai,b\big):\,b\in(uM_i)\Big\}}{<}{r}.
...
\ceq{\hfill\inf\Big\{\varphi\big(\hat ai,b\big)^{M_i}:\,b\in(uM_i)^{|x|}\Big\}}{<}{r}\quad for every $i\in u$.

\ceq{\hfill\inf\Big\{\varphi\big(\hat ai,b\big)^{M_i}:\,b\in(uM_i)^{|x|}\Big\}}{<}{r}\quad for every $i\in u$.

\ceq{\textrm{Therefore,\ for\ some\ }\hat b\in(uN)^{|x|}\hfill \varphi^{M_i}\big(\hat ai,\hat bi\big)}{<}{r}\quad for every $i\in u$.
...
\begin{fact}\label{fact_existential}
  ...
  \begin{itemize}
    \item[1.] there is an $a\in uM$ such that $M\models\ \varphi(a)\le0$;
    \item[2.] $M\models\displaystyle \bigwedge_x\ \varphi(x)\le0$
  \end{itemize}
\end{fact}
\begin{proof}
  Only 2$\Rightarrow$1 requires a proof.
  Let $p(x)=\{\varphi(x)-r: r\in{\mathds R}^+\}$.
...
The limit uniformity on ${\EuScript U}^{|x|}$ is the uniformity that has the following entourages 

\ceq{\hfill V_{\varphi(x),\, \varepsilon}}{=}{\Big\{(a,b)\in{\EuScript U}^{|x|}\times{\EuScript U}^{|x|}\quad :\quad \big|\varphi^{\EuScript U}(a)-\varphi^{\EuScript U}(b)\big|<\varepsilon\Big\}}

for $\varphi(x)\in L({\EuScript U})$ and $\varepsilon\in{\mathds R}^+$. It is the coarsest uniformity that makes all formulas in $L(A)$ uniformly continuous.
...
Throughout the following we fix a saturated model \emph{$\EuScript U$\/} of cardinality $\kappa$, an inaccessible cardinal larger than $|L|$.
(N.B. we always assume $2^\omega\le|L|$.)

\begin{fact}
  Let $k:{\EuScript U}\to{\EuScript U}$ be such that ${\rm dom\,}k\subseteq u\,{\EuScript U}$ has cardinality $<\kappa$.
  Then there is an automorphism $h:{\EuScript U}\to{\EuScript U}$ that extends $k$.
\end{fact}
