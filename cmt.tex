...
\begin{definition}\label{def_1}
  ...
  \footnote{In some examples it may be more natural to use $(n+n'+1)$-sorted structures ${\EuScript M}=\langle  M_1,\dots, M_n,\breve M_1,\dots,\breve M_{n'},\mathds{R}\rangle$. The generalization is straightforeward.} 
  ...
  \item[3.] The (functions that interpret the) terms of sort $ M^n\to\mathds{R}$ have \textit{bounded\/} range.
...
We call $ M$ the \emph{unit ball\/} of ${\EuScript M}$.
...
\begin{definition}
  ...
  \item[ii.] It is closed under the Boolean connectives $\wedge$, $\vee$, and the quantifiers $\forall$, $\exists$ of sort $ M$.
  \item[iii.] It is closed under the quantifier $\forall$ of sort $\mathds{R}$ relativized to any definable subset of $\mathds{R}$.
  \end{itemize}
\end{definition}

Note that in iii of the definition above, definability is intended with respect to the full first order language.
In particular, if $\varphi(x,\varepsilon)\in\mathds{L}(A)$, also $\forall\varepsilon{>}0\;\varphi(x,\varepsilon)\in\mathds{L}(A)$.


\begin{example}[Metric spaces of finite diameter]
  Given a metric space $M,{\rm d}$ of finite diameter we define a structure ${\EuScript M}=\langle  M,\breve M,\mathds{R}\rangle$ as follows.
  Set $M=\breve M$ and let ${\rm id}: M\to \breve M$ be the identical isomorphism (so, $\breve M$ is irrelevant in this example).
  The function symbols are ${\rm id}$ and ${\rm d}$.
...
\begin{example}[Banach spaces]
  ...
  \ceq{\hfill t(x)}{=}{\Big\|\sum_{i=1}^{|x|}\sum_{j=1}^m\lambda_{i,j} x_i\,\Big\|.}

  Therefore, as for all $a\in M^{|x|}$

  \ceq{\hfill t^{{\EuScript M}}(a)}{\le}{\sum_{i=1}^{|x|}\sum_{j=1}^m|\lambda_{i,j}|,}
...
%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%
\section{$\mathds{L}$-elementary relations}

Let ${\EuScript M}$ and ${\EuScript N}$ be models.
We say that $R\subseteq M\times N$ is an \emph{$\mathds{L}$-elementary relation\/} between ${\EuScript M}$ and ${\EuScript N}$ if for every $\varphi(x)\in\mathds{L}$
...
\ceq{1.\hfill a\,\sim_{\EuScript M\!}b}{\Leftrightarrow}{t^{\EuScript M}(a)=t^{\EuScript M}(b)}\ \ for every $t(x)\in\mathds{T}( M)$,

where $|x|=|a|=|b|=1$.

\begin{lemma}
  The relation $(\sim_{\EuScript M\!})\subseteq  M^2$ is an $\mathds{L}$-elementary relation.
  Moreover, it is maximal among these, i.e.\@ no $\mathds{L}$-elementary relation properly contains $(\sim_{\EuScript M\!})$.
\end{lemma}
...
\begin{proof}
  ...
  Note that $a_i\sim_{\EuScript M\!}b_i$ is equivalent to saying that $\Delta\cup\{(a_i,b_i)\}$ is an $\mathds{L}$-elementary relation.
  As $\mathds{L}$-elementary relations are closed under composition $\Delta\cup\big\{(a_1,b_1),\dots,(a_n,b_n)\big\}$ is $\mathds{L}$-elementary.
  It follows that for every $\varphi(x)\in\mathds{L}$

  \ceq{2.\hfill{\EuScript M}\models\varphi(a)}{\Leftrightarrow}{{\EuScript N}\models\varphi(b).}
  
  The converse implication $(2\Rightarrow a\sim_{\EuScript M\!}b)$ is immediate.
\end{proof}


\begin{lemma}
  Let $R\subseteq  M\times N$ be total and surjective $\mathds{L}$-elementary relation.
  Then there is a unique maximal $\mathds{L}$-elementary relation containing $R$.
  This maximal $\mathds{L}$-elementary relation is equal to both $(\sim_{\EuScript M\!})\,R$ and $R\,(\sim_{\EuScript N\!})$.
\end{lemma}
\begin{proof}
  It is immediate to verify that $(\sim_{\EuScript M\!})\,R$ is an $\mathds{L}$-elementary relation containing $R$.
  Let $S$ be any maximal $\mathds{L}$-elementary relation containing $R$.
...
We write ${\rm Aut}({\EuScript M})$ for the set of maximal, total and surjective, $\mathds{L}$-elementary relations $R\subseteq  M^2$.
...
Therefore below we insist in working with $\mathds{L}$-elementary relations in place of $\mathds{L}$-elementary maps.
...
Let $\langle {\EuScript M}_i:i\in I\rangle$ be a sequence of structures, say  ${\EuScript M}_i=\langle  M_i,\breve M_i,\mathds{R}\rangle$, that are \emph{uniformly bounded,} that is, the bounds in 3 of Definition~\ref{def_1} are the same for all ${\EuScript M}_i$.

Let $F$ be an ultrafilter on $I$.

\begin{definition}\label{def_ultraproduct}
  We define a structure \emph{${\EuScript N}=\langle\breve  N,\breve N,\mathds{R}\rangle$\/} that we call the \emph{ultraproduct\/} of the models $\langle{\EuScript M}_i:i\in I\rangle$.
  \begin{itemize}
...
\begin{proof}
  ...
  $\displaystyle\lim_{i\to F}\ t(\hat a\,i)=\beta'$.
  ...
  The rest of the inductive proof is straightforeward and is left to the reader.
\end{proof}
...
\begin{fact}
  ...
  Then $R$ is an $\mathds{L}$-elementary relation.\qed
\end{fact}
...
An \emph{elementary embedding\/} is an $\mathds{L}$-elementary relation that is functional, injective and total.

When $\varnothing$ is an $\mathds{L}$-elementary relation between $ M$ and $\breve N$, the unit balls of ${\EuScript M}$ and ${\EuScript N}$, we say that ${\EuScript M}$ and ${\EuScript N}$ are \emph{elementarily equivalent\/} and write \emph{${\EuScript M}\equiv {\EuScript N}$.}
We write $I_A$ for the diagonal relation on a set $A$.
We write \emph{${\EuScript M}\equiv_A{\EuScript N}$\/} if $I_A$ is an $\mathds{L}$-elementary relation.
In words, we say that ${\EuScript M}$ and ${\EuScript N}$ are $\mathds{L}$-elementary equivalent \emph{over $A$.}
Finally, we write \emph{${\EuScript M}\preceq{\EuScript N}$\/} if ${\EuScript M}\equiv_\breve M{\EuScript N}$ and ${\EuScript M}\subseteq{\EuScript N}$, that is ${\EuScript M}$ is a substructure of ${\EuScript N}$ in the classical sense.
...
\begin{proposition}[Tarski-Vaught test]\label{prop_Tarski-Vaught} Let ${\EuScript M}\subseteq{\EuScript M}$.
  ...
  \begin{itemize}
    ...
    \item[2.] $ M$ is dense in $\breve N$ w.r.t.\@ the $ M\mbox{-}$limit topology on $\breve N$;
    \item[3.] for every $\varphi(x)\in L( M)$, if $\varphi(\breve N)<0$ is non empty, then ${\EuScript N}\models\varphi(b)<0$ for some $b\in  M$.
  \end{itemize}
\end{proposition}
\begin{proof}
  ...
  As $ M$ is a substructure of $\breve N$, this equivalence holds for atomic $\varphi(z)$.
  We leave the case of the propositional connectives to the reader and consider only the case of the infimum quantifier.

  \ceq{\hfill \breve N\,\models\,\smash{\bigwedge_x}\varphi(x,a)<\alpha}{\Leftrightarrow}{} there is a $b\in \breve N$ such that ${\EuScript N}\models\varphi(b,a)<\alpha$
...
% \begin{remark}
% In the Tarski-Vaught test for classical logic it is not necessary to require that $ M$ is a substructure of $\breve N$ as this is a consequence of the test.
% In the uniformly continuous case this is necessary by the lack of a predicate for equality.\qed
...
\end{theorem}
\begin{proof} (\breve Needs some checking.)
...
\begin{fact}
  Let $R\subseteq{\EuScript U}^2$ be an $\mathds{L}$-elementary relation of cardinality $<\kappa$.
  Then there is a total and surjective $\mathds{L}$-elementary relation $S\subseteq{\EuScript U}^2$ containing $R$.
\end{fact}
...
\begin{proof}
  ...
  Let $b\in{\EuScript U}$.
  It suffices to prove that there is a $c$ such that $R\cup\{\langle b,c\rangle\}$ is an $\mathds{L}$-elementary relation.
