...
\item[3.] the functions $n_{\mbox-}:L_{\rm at}\to\mathds N$ and $t_{\mbox-}:L\to L_{\rm trm}$.
\end{itemize}
\end{definition}

The functions in 3 above are called \emph{bounding functions.}
The meaning of $L$, $L_{\rm at}$ and $L_{\rm trm}$ in 4 is clarified below.
...
Again, this can be ignored for the time being.
...
\item[vi.] \noindent\smash{$\displaystyle\big(\bigwedge_x\psi\big)^M\ =\ \inf\big\{\max\big\{0,\psi[x/b]^M\big\} :\ b\in M\big\}$}.
\end{itemize}
\end{minipage}
\end{definition}
\medskip
...
\begin{itemize}
  ...
  \item [3.] Also any {\sc bbhu}-premodel $M$ is a model as those of Section~\ref{uno}.
  A {\sc bbhu}-premodel $M$ is a pseudometric space of diameter $1$.
  There are possibly functions symbols and relations (other than the metric $d(x,y)$).
  All relations take values in the interval $[0,1]$.
  Again, as unit ball take the whole of $M$.
  The function $n_{\mbox-}$ is constant $1$, and $t_{\psi(x,y)}$ is (trivially) the term $y$.
  
  (The following may require some cleaning.)\\
  In a {\sc bbhu}-premodel functins are required to be uniformly continuous with a modulus of uniform continuity fixed by the language.
...
\ceq{\hfill\inf\Big\{\max\big\{0,\ \varphi\big(\hat a,\hat b\big)^N\big\}\ :\ \hat b\in N\Big\}}
{=}
...
First we prove the $\le$ inequality i.e., the run-of-the-mill half of the equality.
Let $r$ be an arbitrary positive real number.

\ceq{\textrm{Assume\ that\ for\ some\ }\hat b\hfill\varphi^N\big(\hat a,\hat b\big)}{<}{r}.

\ceq{\textrm{By\ induction\ hypothesis}\hfill\lim_{i\uparrow F}\varphi^{M_i}\big(\hat ai,\hat bi\big)}{<}{r},

\ceq{\textrm{then\ for\ some\ }u\in F\hfill\varphi^{M_i}\big(\hat ai,\hat bi\big)}{<}{r}\quad for every $i\in u$.

\ceq{\textrm{Hence}\hfill \inf\Big\{\varphi^{M_i}\big(\hat ai,b\big):\,b\in M_i\Big\}}{<}{r}\quad for every $i\in u$.

\ceq{\textrm{Finally,}\hfill\lim_{i\uparrow F}\ \inf\Big\{\varphi^{M_i}\big(\hat ai,b\big):\,b\in M_i\Big\}}{<}{r}.
...
\ceq{\hfill\lim_{i\uparrow F}\inf\Big\{\varphi\big(\hat ai,b\big)^{M_i}:\,b\in M_i^{|x|}\Big\}}
{<}
{r.}

Therefore, for some $u\in F$

\ceq{\hfill\inf\Big\{\varphi\big(\hat ai,b\big)^{M_i}:\,b\in M_i^{|x|}\Big\}}{<}{r}\quad for every $i\in u$.

Without loss of generality we can assume that $\hat a\in U$. 
Hence, writing $t(z)$ for $t_{\varphi(x,y)}(z)$ we have

\ceq{\hfill\inf\Big\{\varphi\big(\hat ai,t(c)\big)^{M_i}:\,c\in U_i^{|z|}\Big\}}{<}{r}\quad for every $i\in u$.

\ceq{\textrm{Therefore,\ for\ some\ }\hat c\in U^{|z|}\hfill \varphi^{M_i}\big(\hat ai,t(\hat ci)\big)}{<}{r}\quad for every $i\in u$.

\ceq{\textrm{Then}\hfill\lim_{i\uparrow F}\varphi^{M_i}\big(\hat ai,t(\hat bi)\big)}{<}{r,}

\ceq{\textrm{and\ finally}\hfill\vphantom{\bigwedge_x}\varphi^N\big(\hat a,t(\hat b)\big)}{<}{r.}
...
\begin{proof}
  ...
  \ceq{}{\Leftrightarrow}{M\,\models\,\bigwedge_x\varphi(x)<\varepsilon.}
...
In the continuous case this is necessary by the lack of equality.\qed
...
Throughout this section $M$ is a model with unit ball $U$ and bounding functions $n_{\mbox{-}},t_{\mbox{-}}$.
...
We say that $p(x)\le 0$ is \emph{uniformly finitely satisfied in $M$\/} if for every formula $\psi(x)$ that is a disjunction of formulas in $p(x)$, there is an $a\in U^{|z|}$ such that $M\models\psi\big(t(a)\big)\le0$, where $t(z)\in L_{\rm trm}$ is a term which only depends on $p(x)$.  

We say that $M$ is \emph{$\lambda\mbox{-}$saturated\/} if for every $p(x)\subseteq L(A)$ as in 1 and 2 below, there is an $a\in M^{|x|}$ such that $M\models p(a)\le0$.
\begin{itemize}
  \item[1.] $A\subseteq M$ has cardinality $<\lambda$;
  \item[2.] $p(x)\le0$ is uniformly finitely satisfied in $M$.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Example: {\sc bbhu} models. (Needs further checking.)}

A {\sc bbhu} (pre)models is (psudo)metric spaces of diameter $1$.
Predicates take vaues in the unit interval and function are uniformly continuous with a modulus of uniform continuity fixed by the language.
That is, for every function symbol $f$, there is a strictly increasing function $\delta:{\mathds R}^+\to {\mathds R}^+$ such that for every $\varepsilon\in{\mathds R}^+$ 

\ceq{\hfill d(x,y)\le\delta(\varepsilon)}{\rightarrow}{d(fx,fy)\le\varepsilon}


Let $N$ be a {\sc bbhu} premodel.
Consider a model $M$ with as predicates $d(x,y)$ and, for every modulus of uniform continuity $\delta$, and evey positive $\varepsilon$, a predicate $r_\varepsilon$ such that 
...
Let $n_{r_\varepsilon(x,y,fx,fy)}=1$, then the function interpreted in the models (as defined in Section~\ref{uno}) are uniformly continuous with modulo $\delta$.

Finally let $t_{\mbox-}$ be trivial: i.e.\@ $t_{\varphi(x,y}=y$ for every $\varphi(x,y)$.
