...
Note that $<$ is a dense (pre)order of $\mathds{L}$.
...
\begin{proof}
  ...
  The step fron terms tu tule of term is clear becouse the topology on $R^n$ is the product topology. 
\end{proof}
...
\begin{proof}
  ...
  Assume $\{i:t^{\EuScript M}(a_i;\alpha)\in C\}\in F$.
  Then $F\mbox{-}\lim_i t^{\EuScript M}(a_i;\alpha)\in C$.
  By Fact~\ref{fact_unifcont}, $F\mbox{-}\lim_i t^{\EuScript M}(a_i;\alpha_i)\in C$.
  Then $\{i:t^{\EuScript M}(a_i\,;\alpha_i)\in D\}\in F$ for every $D$ that is a compact neighborhood of $C$.

  Vice versa, if  $\{i:t^{\EuScript M}(a_i\,;\alpha_i)\in D\}\in F$ for every $D$ that is a compact neighborhood of $C$, then $F\mbox{-}\lim_i t^{\EuScript M}(a_i;\alpha_i)\in C$, and (1) follows by Fact~\ref{fact_unifcont}.
  ...
  {\Leftrightarrow}
  {\big\{ i:{\EuScript M}\models\varphi'(a_i,b_i\,;\alpha_i)\big\}\in F}\quad for every $\varphi'>\varphi$.
  
  We prove

  \ceq{1.\hfill \{i:{\EuScript M}\models\forall y\,\varphi(a_i,y\,; \alpha)\}\in F}
  {\Leftrightarrow}
  {\big\{ i:{\EuScript M}\models\forall y\,\varphi'(a_i,y\,;\alpha_i)\big\}\in F}\quad for every $\varphi'>\varphi$.
  
  Negate the r.h.s.\@ of the implication.
  Then there are a formula $\varphi'<\varphi$ and a sequence $\langle b_i:i\in I\rangle$ such that (using that $F$ is an ultrafilter)
  ...
  Induction for the quantifier $\forall^C$ is also virtually identical, but we repeat the argument for convenience.
  ...
  Induction for the quantifier $\exists^C$ is proved by the dual version of argument above.
\end{proof}
...
In fact, in the more general setting of ultraproducts, it would not hold without the requirement above.
...
Its first application is below, in the proof of a version of the classical Tarski-Vaught test.
...
\begin{lemma}
  ...
  \begin{itemize}
    ...
    \ceq{\hfill{\EuScript N}\models\exists x\,\neg\varphi(x)}{\Rightarrow}{\textrm{ there is an }a\in A\textrm{ such that }{\EuScript N}\models\neg\varphi(a);}
 
    \item[2.] for every formula $\varphi(x)\in\mathds{L}(M)$
    
    \ceq{\hfill{\EuScript N}\models\exists x\,\varphi(x)}{\Rightarrow}{\textrm{for every }\varphi'>\varphi\textrm{ there is an }a\in A\textrm{ such that }{\EuScript N}\models\varphi'(a).}
 
  \end{itemize}
  
\end{lemma}

\begin{proposition}[$\mathds{L}$-Tarski-Vaught Test]
  Assume the two equivalent properties of the lemma above.
  Then $A$ is the domain of a structure ${\EuScript M}\preceq^\mathds{L}{\EuScript N}$.
\end{proposition}
...
\begin{proof}
  ...
  Therefore, form every formula as in (i) and (ii) of Definition~\ref{def_LL} we have
  ...
  \ceq{}
  {\Rightarrow}
  {\textrm{for every }\varphi'>\varphi\textrm{ there is an }b\in M^{|y|}\textrm{ such that }{\EuScript M}\models\varphi'(a,b)}
...
\begin{proof}
  Assume the fact for $\lambda$ and let $a_\lambda\sim b_\lambda$.
