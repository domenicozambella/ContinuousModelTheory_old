...
The set $\mathds{H}$ is the immediate generalization of the set of  positive bounded formulas of Henson and Iovino.
We also introduced the class $\mathds{L}$ because it offers some advantages. 
For instance, it is easy to see that $\mathds{L}$ is at least as expressive as of real valued logic (in fact, way more).
...
\begin{proof}
  ...
  By the local compactness of $R$, there are $C'$ and $O$, disjoint neighborhoods of $C$ and $\tilde{C}$ respectively, where is $C'$ is compact.
...
A theory $T$ is consistent if ${\EuScript M}\models T$ for some  model  ${\EuScript M}$ satisfying Definition~\ref{def_0}.
Also the notion of logical consequence is restricted to models in this class. 

As in the classical case, a theory $T$ is said to be finitely consistent if every $\varphi$, conjunction of sentences in $T$, is consistent.
...
\begin{proposition}[Compactness Theorem]\label{prop_compattezza}
  Let $T\subseteq\mathds{L}$ be a uniformly finitely consistent theory.
