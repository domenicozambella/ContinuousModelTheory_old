...
These notes have no ambition of mathematcal novelty.
In fact, most techniques applied here are at least half a century old.
Still we believe that an important facet of mathematics progress consists of the simplificaytion and clarification of existent knowledge.
We hope to contribute in this manner.
...
Real vector spaces are among the most simple strucures considered in model theory.
Unfortunately, in most situations they are not expressive enough.
It is natural to expand them by adding a norm. 
The norm is a function that, given vector, outputs a real number.
Unfortunately, this function messies up with our intuition of elementary extension.
Ideally, we would like to extend the vector field while mantaining the usal notion of real number\footnote{Non standard standard analists have no problem in expanding $\mathds{R}$. In these notes, for a change, we insist as $\mathds{R}$ needs to remain $\mathds{R}$ throughout.} but ths is impossible.
...
\begin{definition}\label{def_0}
  ...
  Function symbols of $L'$ falls in one of these two groups
  \begin{itemize}
    \item[1.] $M^n\times R^m\to M$
    \item[2.] $M^n\times R^m\to R$
  \end{itemize}

  We require that function symbols of the second group are interpreted in functions such that
  \begin{itemize}
...
As a remedy, below we carve out a set $\mathds{L}$ of formulas, $L\subseteq\mathds{L}\subseteq L'$, such that every model as in Definition~\ref{def_0} has an $\mathds{L}$-elementary extension that is $\mathds{L}$-saturated and is still same class.
Our choice of $\mathds{L}$ is not the most general.
It is a compromise that avoids eccessive complications.

In a nutshell, we require that function symbols of the second group above do not occur under the scope of a negation.
We also do not allow existential quantifiers of sort $R$.
In fact, the ultrapower construction that we present in Section~\ref{ultrapws} is completely classical w.r.t.\@ functions the first group. 
On the other hand, functions of the second group are \textit{critical\/} and need to be dealt with differently.
...
\begin{definition}\label{def_LL}
  ...
  \item[ii.] quantifier free formulas with only terms in $\mathds{T}_M$.
  \end{itemize}
  We require that $\mathds{L}$ is closed under the Boolean connectives $\wedge$, $\vee$, the quantifiers $\forall$, $\exists$ of sort $M$; and the universal quantifier $\forall$ of sort $R$.
\end{definition}
...
\begin{definition}\label{def_unif_cont}
  Let $I$ be a pure set. Let $X,Y$ be topological spaces, and assume $Y$ is Hausdorff and compact.  
...
\begin{lemma}[???]
  ...
  For a given $F\in\beta I$ define $\alpha=F\mbox{-}\lim_iy_i$.
  Then $f(F,\alpha)=F\mbox{-}\lim_i f(i,y_i)$.
\end{lemma}


\begin{lemma}[???]
  Let $f:I\times X\to Y$ be continuous uniformly in $I$.
  Let $g:I\times Y\to I$ be any function and let $\bar g:\beta I\times Y\to \beta I$ the unique continuous extension of $g$.
  Let $\langle y_i\in Y : i\in I\rangle$.
  For a given $F\in\beta I$ define $\alpha=F\mbox{-}\lim_iy_i$.
...
\begin{proof}
  ...
  By induction hypothesis, and the requirement that $F$ is an ultrafilter, $\big\{ i\in I\ :\ {\EuScript M}\not\models\varphi'(\hat ai;\alpha,\beta)\big\}\in F$ for some $\varphi'>\varphi$.
