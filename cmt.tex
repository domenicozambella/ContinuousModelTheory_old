...
\hfill\texttt{Branch:\ debole\ \DTMnow}\bigskip
\maketitle
\raggedbottom
...
Ideally, we would like to have that an elementary extension of a normed space mantain the usal notion of real numbers\footnote{Non standard standard analists have no problem in expanding $\mathds{R}$. In these notes, for a change, we insist as $\mathds{R}$ should remain $\mathds{R}$ throughout.} but this is not possible if we insist to mantain the classical notion of elementary extension.
...
\begin{fact}\label{fact_unifcont}
  ...
  \begin{itemize}
    \item $t(x;y)$, an $n$-tuple of terms of sort $M^{|x|}\times R^{|y|}\to R$;
    \item $\langle a_i:i\in I\rangle$, a sequence of elements $a_i\in M^{|x|}$;
    \item $U\subseteq R^n$ an open set;
    \item $F$, an ultrafilter on $I$.
  \end{itemize}
  Then for every $\alpha\in R^{|y|}$ the following are equivalent
  \begin{itemize}
    \item[1] \noindent\kern-4ex  \ceq{\hfill\{i\in I : t^{\EuScript M}( a_i;\alpha)\in U\}}{\in}{F.}
    \item[2] there is an open $V\ni\alpha$ such that for every $\alpha'\in V$
...
\begin{proof}
  Only (1$\Rightarrow$2) requires a proof.
  By induction on the nesting of functions of sort $R^n\to R$
  In the basis case the tuple $t$ is composed of elements of $R$ and terms of sort $M^n\to R$.
  Then the fact is clear.
  Assume inductively that the fact holds for the tuple $t(x,y)$ and prove it for the tuple $f_1\big(t(x,y)\big),\dots,f_n\big(t(x,y)\big)$.
  It suffices to prove it for $n=1$.
  Hence, assume

  \ceq{\hfill\Big\{i\in I : f^{\EuScript M}\big(t^{\EuScript M}( a_i;\alpha)\big)\in U\Big\}}{\in}{F}

  or equivalently

  \ceq{\hfill\Big\{i\in I : t^{\EuScript M}(a_i;\alpha)\in \big(f^{\EuScript M}\big)^{-1}[U]\}}{\in}{F.}
  
  By the continuity of the functions of sort $R^n\to R$, the required $V$ is obtained from the induction hypothesis. 
\end{proof}
...
\begin{lemma}\label{lem_HIapprox}
  ...
  \begin{itemize}
    \item[1]\noindent\kern-4ex   \ceq{\hfill\{i\in I:{\EuScript M}\models\varphi(a_i;\alpha)\}}{\in}{F.} 
...
\begin{proof}
  ...
  Assume that $\{i:{\EuScript M}\models t(a_i;\alpha)\in C\}\in F$.
  Then $F\mbox{-}\lim_i t^{\EuScript M}(a_i;\alpha)\in C$.
  Let $D$ be a compact neighborhood of $C$.
  By Fact~\ref{fact_unifcont}, there is an open $V\ni\alpha$ such that 
  $\{i:{\EuScript M}\models t(a_i;\alpha')\in D\}\in F$ for every $\alpha'\in V$.
  Then $\{i:{\EuScript M}\models t'(a_i\,;\alpha_i)\in D\}\in F$ follows.
  As $D$ is arbitrary, (1) follows.
  Vice versa, using that $F$ is an ultrafilter, assume that $\{i:{\EuScript M}\not\models t(a_i;\alpha)\in C\}\in F$. 
  By local compactness and regularity, $\{i:{\EuScript M}\not\models t(a_i;\alpha)\in D\}\in F$ for some $D$, a compact neighborhood of $C$.
  Then the negation of (2) follows.
  ...
  \ceq{\hfill \big\{i:{\EuScript M}\models\varphi(a_i,b_i\,; \alpha)\big\}\in F}
  {\Leftrightarrow}
  {\big\{ i:{\EuScript M}\models\varphi'(a_i,b_i\,;\alpha_i)\big\}\in F}.
  
  We prove

  \ceq{1.\hfill \{i:{\EuScript M}\models\forall y\,\varphi(a_i,y\,; \alpha)\}\in F}
  {\Leftrightarrow}
  {\big\{ i:{\EuScript M}\models\forall y\,\varphi'(a_i,y\,;\alpha_i)\big\}\in F}
  
  Negate the r.h.s.\@ of the implication.
  Then there is a sequence $\langle b_i:i\in I\rangle$ such that (using that $F$ is an ultrafilter)

  \ceq{}
  { }
  {\big\{ i:{\EuScript M}\not\models\varphi'(a_i,b_i\,;\alpha_i)\big\}\in F}
  
  Assume the l.h.s.\@ of (1) and reason for a contradiction.
  Then $\big\{i:{\EuScript M}\models\varphi(a_i,b_i\,; \alpha)\big\}\in F$ and, by induction hypothesis, $\big\{ i:{\EuScript M}\models\varphi'(a_i,b_i\,;\alpha_i)\big\}\in F$. A contradiction that proves the implication ($\Rightarrow$) in (1). The converse implication is immediate.
  
  Induction for the existential quantifier of sort $M$ is similar (dual) to the proof for the universal quantifier.

  Induction for the quantifier $\forall^B$ is also virtually identical, but we repeat the argument for convenience.
  Assume inductively that for every $\varphi'>\varphi$ and every sequence $\langle\beta_i:i\in I\rangle$ in $B$ such that $\beta=F\mbox{-}\lim_i\beta_i$

  \ceq{\hfill \big\{i:{\EuScript M}\models\varphi(a_i\,; \alpha,\beta)\big\}\in F}
  {\Leftrightarrow}
  {\big\{ i:{\EuScript M}\models\varphi'(a_i\,;\alpha_i,\beta_i)\big\}\in F}.
  
  We prove

  \ceq{2.\hfill \{i:{\EuScript M}\models\forall^C\!y\,\varphi(a_i,; \alpha,y)\}\in F}
  {\Leftrightarrow}
  {\big\{ i:{\EuScript M}\models\forall^C\!y\,\varphi'(a_i\,;\alpha_i,y)\big\}\in F}
  
  Negate the r.h.s.\@ of the implication.
  Then there is a sequence $\langle\beta_i:i\in I\rangle$ such that (again using that $F$ is an ultrafilter)

  \ceq{}
  { }
  {\big\{ i:{\EuScript M}\not\models\varphi'(a_i\,;\alpha_i,\beta_i)\big\}\in F}
  
  As $B$ is compact, $F\mbox{-}\lim_i\beta_i$ exists, say it equals $\beta\in B$.
  Assume the l.h.s.\@ of the implication and reason for a contradiction.
  Then $\big\{i:{\EuScript M}\models\varphi(a_i\,; \alpha,\beta)\big\}\in F$ and, by induction hypothesis, $\big\{ i:{\EuScript M}\models\varphi'(a_i\,;\alpha_i,\beta_i)\big\}\in F$. A contradiction that proves the implication ($\Rightarrow$) in (2) . 
  The converse implication is immediate.

  Induction for the quantifier $\exists^B$ is proved by the dual version of argument above.
\end{proof}
...
We require that for each function symbol $f$ of sort $M^n\to R$ there is a compact $C\subseteq R$ that contains the range of all the functions $f^{{\EuScript M}_i}$.
...
\begin{definition}\label{def_ultraproduct}
  ...
  \begin{itemize}
    ...
    $$
    \item[6.] The interpretation of functions of sort $R^n$ remains unchanged.
  \end{itemize}
\end{definition}

The following is immediate but it needs to be noted.
In fact, in the more general setting of ultraproducts, it would not hold without the uniformity assumption above.
...
\begin{proof}
  By induction on the syntax.
  If $\varphi(x;y)\in L$ then the theorem reduces to the classical \L\v{o}\'s Theorem. Then, suppose that $\varphi(x;y)$ is as in (i) of Definition~\ref{def_LL}, say it is the formula $t(x;y)\in C$.
  ...
  By local compactness of $R$ there is a compact neighborhood of $C$, such that $t^{\EuScript N}(\hat a,\alpha)\notin D$. 
  ...
  % Now, we prove (the argument is dual to the one above -- it is repeated for convinience)
  ...
  Induction for the quantifier $\exists^C$ is again dual to the proof above.
\end{proof}
...
The following fact is proved as in the classical case using that ${\EuScript M}\models\varphi'$ for every $\varphi'>\varphi$ if and only if ${\EuScript M}\models\varphi$.
...
The following is proved as in the classical case.

\begin{proposition}[Tarski-Vaught Test]
  The following are equivalent 
  \begin{itemize}
    \item[1.] ${\EuScript M}\preceq^\mathds{L}{\EuScript N}$
    \item[2.] $M$ is a subset of $N$ and for every formula $\varphi(x)\in\mathds{L}(M)$
    
    \ceq{\hfill{\EuScript N}\models\exists x\,\varphi(x)}{\Rightarrow}{{\EuScript N}\models\varphi(a)} \ for some $a\in M$\hfill\qedsymbol
  \end{itemize}
\end{proposition}
...
We say that $p(x)$ is \emph{finitely satisfied in ${\EuScript M}$\/} if ${\EuScript M}\models\exists x\,\varphi'(x)$ for every $\varphi'>\varphi$ where $\varphi(x)$ is any conjunction of formulas in $p(x)$.
...
Then proof that every model embeds $\mathds{L}$-elementarily in an $\mathds{L}$-saturated one proceeds as in the classical case.
First, note that the classical ultrapower construction yieds the the following lemma.
...
Finally, using a sufficiently long $\mathds{L}$-elementary chain we obtain the required saturated extension.
...
\begin{proof}
  Assume inductively that the fact holds for $\lambda$ and let $a_\lambda\sim b_\lambda$.
