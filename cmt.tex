...
As a remedy, below we carve out a set $\mathds{L}$ of formulas, $L\subseteq\mathds{L}'$, such that every model as in Definition~\ref{def_0} has an $\mathds{L}'$-elementary extension that is $\mathds{L}'$-saturated in the same class.
...
\begin{proof}
  ...
  We claim that that the following implications hold for every $\varphi'>\varphi$
  ...
  A contradiction that proves (\textsf{i}$\forall$).
...
\def\ceq#1#2#3{\parbox[t]{45ex}{$\displaystyle #1$}\parbox{5ex}{\hfil $#2$}{$\displaystyle #3$}}
...
\begin{proposition}[$\mathds{L}$\llap{\rotatebox{30}{--}\kern1.5pt}\v{o}\'s Theorem]\label{thm_Los}
  ...
  Then for all $\hat a\in N^{|x|}$ the following are equivalent
  \begin{itemize}
