...
\begin{definition}\label{def_LL}
  ...
  \begin{itemize}
  \item[i.] formulas as $t(x;y)\in C$, where $C\subseteq R^n$ is compact\footnote{We confuse the relation symbols in $L'$ of sort $R^m$ with their interpretation and write $t\in C$ for $C(t)$} and $t(x,y)$ is a tuple of terms of sort $M^{|x|}\times R^{|y|}\to R$; 
...
\begin{fact}\label{fact_unifcont}
  Let $t(x;y)$ be a tuple of terms of sort $M^{|x|}\times R^{|y|}\to R$.
  Let $U\subseteq R^{|y|}$ be an open set. 
  Assume that for some $\alpha\in R^{|y|}$ and some sequence $\langle a_i:i\in I\rangle$ of elements of $M^{|x|}$ and 
...
\begin{lemma}\label{lem_HIapprox}
  Let $\alpha=F\mbox{-}\lim_i\alpha_i$, where $\langle \alpha_i:i\in I\rangle$ is a sequence in $R^{|y|}$ and $F$ is an untrafilter on $I$.
  Let  $\langle a_i:i\in I\rangle$ be a sequence in $M^{|x|}$.
  Let $\varphi(x;y)\in\mathds{L}$ be such that $\{i:{\EuScript M}\models\varphi(a_i;\alpha)\}\in F$. 
  Then $\{i:{\EuScript M}\models\varphi'(a_i\,;\alpha_i)\}\in F$ for every $\varphi'>\varphi$.
\end{lemma}
\begin{proof}
  ...
  Using that $F$ is an ultrafilter, there is a sequence $\langle b_i:i\in I\rangle$ such that 
...
\begin{definition}\label{def_ultraproduct}
  ...
  \begin{itemize}
    ...
    \item[3.] The interpretation of functions of sort $R^n\to  R$ is unchanged.
    ...
    $$
    \item[6.] The interpretation of functions of sort $R^n$ is unchanged.
  \end{itemize}
\end{definition}
...
\begin{fact}\label{fact_easy_induz}
  If  $t(x)$ is a term of type $M^{|x|}\to M$ 
...
\begin{fact}\label{fact_termini2}
  If  $t(x;y)$ has sort $M^{|x|}\times R^{|y|}\to R$ 
