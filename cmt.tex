...
The following follows from \L\v{o}\'s Theorem just as in the classical case.
...
The definitions \emph{$\mathds{H}$-elementary\/} map/embedding/substructure is similar.
Note that a total and surjective $\mathds{H}$-elementary map is an ${\EuScript L}$-isomorphism.


\begin{proposition}
  Let $f:M\to N$ be an $\mathds{L}$-elementary map.
  Then for every $\varphi(x)\in\mathds{L}$ and every $a\in({\rm dom }f)^{|x|}$

  \ceq{\hfill{\EuScript M}\models\varphi'(a)}{\Leftrightarrow}{{\EuScript N}\models\varphi'(fa)}
  
  for every $\varphi'>\varphi$. The same holds with $\mathds{H}$ for $\mathds{L}$
\end{proposition}
...
As in the classical case, a theory $T$ is said to be finitely consistent if every conjunction of sentences in $T$ is consistent.
...
\ceq{\hfill\mathds{H}\mbox{-tp}(a/A)}{=}{\big\{\varphi(x)\ :\ \varphi(x)\in\mathds{H}(A)\textrm{ such that }\varphi(a)\big\}}
 ...
 We write  $S_{\mathds{H},x}(A)$ if we fix the tuple of variables.
...
\begin{proposition}\label{prop_negphi_approx}
