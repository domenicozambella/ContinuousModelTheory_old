...
We write $\tilde{\varphi}\perp\varphi$ if $\tilde{\varphi}$ is obtained by replacing each atomic formula $t\in C$ occurring in $\varphi$ with $t\in C'$ where $C'$ is some compact set disjoint from $C$. 
All other atomic formula (i.e.\@ those in $L$) are replaced with their negation.
Moreover, each connective is replaced with its dual.
...
Therefore we call  $\tilde{\varphi}$ a \emph{strong negation} of $\varphi$
...
A total $\mathds{L}$-map is called an \emph{$\mathds{L}$-(elementary) embedding.} 
If natural immersion ${\rm id}_M:M\hookrightarrow N$ is an $\mathds{L}$-embedding, that is, if for every  $\varphi(x)\in\mathds{L}$
