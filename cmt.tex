...
\begin{definition}
  We define the set of formulas o \emph{$\mathds{L}(A)$\/} inductively as follows
  \begin{itemize}
...
Note that in iii of the definition above, definability is intended with respect to the full first order language of ${\EuScript M}$ which is strictly larger than $\mathds{L}(A)$.
...
For every tuple $t(x)=t_1(x),\dots,t_n(x)$ of terms in $\mathds{T}(M)$, with $|x|=1$, we define a pseudometric ${\rm d}_t$ on $M$ as follows
...
If $\varphi(x)\in\mathds{L}(A)$ and $\varepsilon>0$ we write \emph{${}^{\varepsilon\kern-2pt}\varphi(x)$\/} for the formula obtained by replacing every occurence in $\varphi$ of the atomic formula $t(x)\le0$ with $t(x)-\varepsilon\le0$ or, less petandically, $t(x)\le\varepsilon$.
We write \emph{${}^{\neg\varepsilon\kern-2pt}\varphi(x)$\/} for the formula obtained by replacing $t(x)\le0$ with $\varepsilon\le t(x)$.

Note that ${}^{\varepsilon\kern-2pt}\varphi(x),\ {}^{\neg\varepsilon\kern-2pt}\varphi(x)\in\mathds{L}(A)$.
...
Let $a\in U^{|x|}$.
We say that ${\EuScript M}$ \emph{isolates $a$ in the limit\/} if 

\ceq{\hfill p_{\restriction M}(x)}{\vdash}{p(x):={\rm tp}_\mathds{L}(a/U)}

or, in other words, if $a'\equiv^\mathds{L}_Ma$ implies $a'\sim_{\EuScript U}a$.

Note that the isolation of a tuple is equivalent to the isolation of its components.
...
Now we characterize isolation in the limit in topological terms.

\begin{proposition}
  The following are equivalent for every $a\in U$
  \begin{itemize}
    \item[1.] ${\EuScript M}$ isolates $a$ in the limit;
...
\begin{proof}
  ...
  (1$\Rightarrow$2) \ 
  The set of metrics ${\rm d}_t$ defined in Section~\ref{L-relations} is a directed set under the relation of refinement.
  For every such metric ${\rm d}_t$ and every $\varepsilon>0$  we have that 

  \ceq{\hfill p_{\restriction M}(x)}{\vdash}{{\rm d}_t(x,a)\le\varepsilon}

  By compactness, for some $\varphi(x)\in\mathds{L}(M)$

  \ceq{\hfill \varphi(x)}{\rightarrow}{{\rm d}_t(x,a)\le\varepsilon}

  Therefore there is some $b_t\in M$ such that ${\rm d}_t(b_t,a)\le\varepsilon$.
  This defines a net of elements of $M$ that, by definition, converges to $a$.
  ...
  Hence $p_{\restriction M}(x)\vdash p(x)$.
\end{proof}

\begin{definition}
  A model ${\EuScript M}$ is complete if it contains, up to $(\sim_{\EuScript U})$-equivalence, all points it isolates in the limit.
\end{definition}

Let \emph{$\bar M$\/} be the set points that are isolated in the limit by ${\EuScript M}$.
We denote by \emph{$\bar{\EuScript M}$\/} the (unique) substructure of ${\EuScript U}$ with unit ball $\bar M$.
We call  $\bar{\EuScript M}$ this the \emph{completion\/} of ${\EuScript M}$.

\begin{proposition}
  $\bar{\EuScript M}$ is a complete model.
\end{proposition}

\begin{proof}
  We first prove that $\bar{\EuScript M}$ is complete.
  Suppose $a\in U$ is isolated in the limit by $\bar{\EuScript M}$, that is

  \ceq{\hfill p'(x,b)}{\vdash}{p(x):={\rm tp}_\mathds{L}(a/U)}
  ...
  \ceq{\hfill q_{\restriction M}(z)\ \cup\ p'(x,z)}{\vdash}{p(x)}
  
  Finally, $a\in\bar M$ follows because

  \ceq{\hfill p_{\restriction M}(x)}{\vdash}{\exists z\ \big[q_{\restriction M}(z)\ \cup\ p'(x,z)\big].}
  
  Now, we that $\bar{\EuScript M}\preceq_\mathds{L}{\EuScript U}$ using the Tarski-Vaught test.
  Suppose $\exists x\,\varphi(x,b)$ holds in ${\EuScript U}$, where $\varphi(x,z)\in\mathds{L}(M)$ and $b\in\bar M^{|z|}$.
...
% Let $a\in U^{|x|}$.
% We say that ${\EuScript M}$ \emph{isolates $a$ in the limit\/} if there is a formula $\varphi(x)\in\mathds{L}(M)$ and some infinite sets $B_1,\dots,B_n\subseteq M$ such that

% \ceq{\hfill\big\{\varphi(x)\big\}\ \cup\ p_{\restriction C}(x)}{\vdash}{p(x):={\rm tp}_\mathds{L}(a/U)}
...
% \begin{proposition}
%   Suppose that $a'\equiv^\mathds{L}_Ma$ are both isolated in the limit by ${\EuScript M}$.
...
% \begin{proof}
%   Let $a\in U$ be isolated in the limit by $\bar{\EuScript M}$.
...
%   \ceq{\hfill\big\{\varphi(x,b)\big\}\ \cup\ p_{\restriction C}(x,b)}{\vdash}{p(x,b).}

%   where $p(x,z)={\rm tp}_\mathds{L}(a,b/U)$.
%   As $b$ is isolated in the limit by ${\EuScript M}$, there is a 

%   \ceq{\hfill\big\{\psi(z)\big\}\ \cup\ q_{\restriction C}(z)}{\vdash}{q(z)={\rm tp}_\mathds{L}(b/U)}
% \end{proof}
...
%   \ceq{\hfill\big\{\varphi(z)\big\}\ \cup\ p_{\restriction C}(z)}{\vdash}{\exists x\ \psi(x, z)}
% \end{proof}
...
\begin{proof}
  ...
  We claim that $q(x,\bar a)$ is finitely consistent.
