...
\begin{example}[Metric spaces of finite diameter]\label{ex_metric}
  ...
  Set $M=\breve M$ and let ${\rm id}: M\to \breve M$ be the identical isomorphism (hence $\breve M$ is pleonastic in this case).
...
Equality between elements in the unit ball is not among the formulas in $\mathds{L}$.
For this reason, relations, rather than partial maps, are the natural tool to discuss elementarity. 
...
\begin{lemma}
  ...
  This maximal $\mathds{L}$-relation is equal to both $(\sim_{\EuScript M\!})\,R$ and $R\,(\sim_{\EuScript N\!})$.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  Induction for the connectives $\wedge$ and $\vee$ is straightforeward.
...
Fix a model ${\EuScript M}$ with unit ball $M$.
Let ${\EuScript U}$ be a saturated extension of ${\EuScript M}$ with unit ball $U$.
Let $a\in U$.
We say that ${\EuScript M}$ \emph{isolates $a$ in the limit\/} if the are a finite set $A\subseteq M$ and an infinite set $A\subseteq B\subseteq M$ such that for every infinite $A\subseteq C\subseteq B$

\ceq{\hfill p_{\restriction C}(x)}{\vdash}{p(x):={\rm tp}_\mathds{L}(a/U)}

The following proposition is immediate consequence of the definition of the relation $(\sim_{\EuScript U})$, see Section~\ref{L-relations}.

\begin{proposition}
  Suppose that $a'\equiv^\mathds{L}_Ma$ are isolated in the limit by ${\EuScript M}$.
  Then $a'\sim_{\EuScript U}a$.
\end{proposition}

Finally we define completeness which is a form of saturation whose strength lies between $\omega$-$\mathds{L}$-saturation and $\omega_1$-$\mathds{L}$-saturation.

\begin{definition}
  A model ${\EuScript M}$ is complete if it contains all points isolated in the limit.
  The completion of ${\EuScript M}$ is the minimal model ${\EuScript M}'$ that contains all points isolated by ${\EuScript M}'$.
  It is easy to verify that ${\EuScript M}'$ is complete.
\end{definition}

It is easy to verify that if ${\EuScript M}$ is the metric space in Example~\ref{ex_metric}, then the completeness of ${\EuScript M}$ implies the Cauchy-completeness of $(M,\,{\rm d})$.
