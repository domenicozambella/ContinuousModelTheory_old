...
\begin{definition}\label{def_1}
  ...
  \footnote{In some examples it may be more natural to use $(n+n'+1)$-sorted structures ${\EuScript M}=\langle M_1,\dots,M_n,\dot M_1,\dots,\dot M_{n'},\mathds{R}\rangle$. The generalization is straightforeward.} 
  structures of the form ${\EuScript M}=\langle M,\dot M,\mathds{R}\rangle$.
  ...
  \begin{itemize}
    ...
    \item[b.] $M^n\times{\dot M}^m\to\textrm{any of } M,\ \dot M,\textrm{ or }\mathds{R}$
    % \item[c.] $M^n\times{\dot M}^m\to\dot M$
    % \item[d.] $M^n\times{\dot M}^m\to\mathds{R}$
  \end{itemize}
  ...
  \item[4.] The relation have sort $\mathds{R}^n$ and are \textit{closed\/} in the usual topology.
  \end{itemize}
\end{definition}
...
For simplicity we assume it contains \textit{all\/} functions and relations as in 2 and 4 above.

\begin{definition}
  We write \emph{$\mathds{T}(A)$\/} for the set of terms of sort $M^n\to\mathds{R}$ or $\mathds{R}^n\to\mathds{R}$ and parameters in some set $A\subseteq M$.
  We write \emph{$\mathds{L}(A)$\/} for the set of formulas obtained inductively as follows
  \begin{itemize}
  \item[i.] $\mathds{L}(A)$ contains the atomic formula $r(t)$ for every relation symbol $r$ of sort $\mathds{R}^n$, and every $n$-tuple $t$ of terms in $\mathds{T}(A)$.
  \item[ii.] $\mathds{L}(A)$ is closed under the Boolean connectives $\wedge$, $\vee$, and the quantifiers $\forall$, $\exists$ of sort $M$.
  \item[iii.] $\mathds{L}(A)$ is closed under the quantifier $\forall$ of sort $\mathds{R}$.
  We are also allowed to restrict the range of $\forall$ to \textit{any\/} definable subset of $\mathds{R}$.
  \end{itemize}
\end{definition}

E.g.\@ iii asserts, in particular, that if $\varphi(x,\varepsilon)\in\mathds{L}(A)$ then also $\forall\varepsilon{>}0\;\varphi(x,\varepsilon)\in\mathds{L}(A)$.

\begin{example}[Banach spaces]
  Given a Banach space  $\dot M$ we define a structure ${\EuScript M}=\langle M,\dot M,\mathds{R}\rangle$ as follows.
  Let $M=\{a\in \dot M: \|a\|\le1\}$ be the closed unit ball of $\dot M$.
  Besides the symbols mentioned above, $\mathds{L}$ contains a function symbol for the natural embedding ${\rm id}:M\to \dot M$.
  It contains also a symbol for the norm $\|\mbox-\|:\dot M\to\mathds{R}$.
  Finally, $\mathds{L}$ contains the usual symbols of the language of vector spaces.
  These have sort ${\dot M}^n\to \dot M$, for the appropiate $n$.

  Finally, note that 2 above is immediate.
  In fact, 2 only mentions terms with domain $M$ and ignores those with domain $\dot M$.
\end{example}
...
Let $\langle {\EuScript M}_i:i\in I\rangle$ be a sequence of structures, say  ${\EuScript M}_i=\langle M_i,\dot M_i,\mathds{R}\rangle$, that are uniformly bounded.
That is, the bounds in 3 of Definition~\ref{def_1} are the same for all ${\EuScript M}_i$.

Let $F$ be an ultrafilter on $I$.

\begin{definition}\label{def_ultraproduct}
  We define a structure \emph{${\EuScript N}=\langle N,\dot N,\mathds{R}\rangle$\/} that we call the \emph{ultraproduct\/} of the models $\langle{\EuScript M}_i:i\in I\rangle$.
  \begin{itemize}
    \item[1.] $N$ is the set of the sequences $\hat a: I\to\bigcup_{i\in I}M_i$ such that $\hat a\,i\in M_i$.
    \item[2.] $\dot N$ is the set of the sequences $\hat c: I\to \bigcup_{i\in I}\dot M_i$ such that $\hat c\,i = \dot M_i$.
    \item[3.] If $f$ is a function of sort $M^n\times{\dot M}^m\to M$ then $f^{\EuScript N}(\hat ai,\hat ci)$ is the sequence 
    $f^{{\EuScript M}_i}(\hat ai,\hat ci)$.
    \item[4.] Similarly if $f$ is of sort $M^n\times{\dot M}^m\to\dot M$.
    \item[5.] Finally, if $f$ is a function of sort $M^n\times{\dot M}^m\to\mathds{R}$ then 
    $$
    f^{\EuScript N}(\hat ai,\hat ci)=\lim\limits_{i\uparrow F}\ f^{{\EuScript  M}_i}(\hat ai,\hat ci).
    $$  
  \end{itemize}
  As usual, if ${\EuScript M}_i={\EuScript M}$ for all $i\in I$, we say that ${\EuScript N}$ is an \emph{ultrapower\/} of ${\EuScript M}$.
\end{definition}

The limit in 5 of the definition above exists because $F$ is an ultrafilter and it is finite because all models ${\EuScript M}_i$ have the same bounds.
