...
\begin{example}[Banach spaces]
  Given a Banach space  $\breve M$ we define a structure ${\EuScript M}=\langle M,\breve M,\mathds{R}\rangle$ as follows.
  Let $M=\{a\in \breve M: \|a\|\le1\}$ be the closed unit ball of $\breve M$.
  Besides the symbols mentioned above, $\mathds{L}$ contains a function symbol for the natural embedding ${\rm id}:M\to \breve M$.
  It contains also a symbol for the norm $\|\mbox-\|:\breve M\to\mathds{R}$.
...
If $r:I\to{\mathds R}$ and $\lambda\in{\mathds R}\cup\{\pm\infty\}$ we write
...
When $F$ is an ultrafilter, such a $\lambda$ always exists.

Let $I$ be an infinite set.
Let $\langle {\EuScript M}_i:i\in I\rangle$ be a sequence of structures, say  ${\EuScript M}_i=\langle M_i,\breve M_i,\mathds{R}\rangle$, that are uniformly bounded.
...
\begin{definition}\label{def_ultraproduct}
  ...
  \begin{itemize}
    ...
    \item[2.] $\breve N$ is the set of the sequences $t(\hat a)$ of the form $t^{{\EuScript M}_i}(\hat ai)$, where $t(x)$ is a term of sort $M^n\to\breve M$.
    \item[3.] If $f$ is a function of sort $M^n\times{\breve M}^m\to M$ then $f^{\EuScript N}(\hat a,t^{{\EuScript M}_i}\big(\hat c)\big)$ is the sequence 
    ...
    $$
    f^{\EuScript N}\big(\hat a,t(\hat c)\big)\ =\ \lim_{i\uparrow F}f^{{\EuScript  M}_i}\big(\hat ai,t^{{\EuScript M}_i}(\hat ci)\big).
    $$ 
  \end{itemize}
...
\begin{proposition}[\L\v{o}\'s Theorem]
  Let ${\EuScript N}$ be as above and let $\varphi(x)\in\mathds{L}(A)$. Then for every $\hat a\in N^{|x|}$ and $\lambda\in\mathds{R}$

  \ceq{\hfill {\EuScript N}\models\varphi(\hat a, \lambda)}
  {\Leftrightarrow}
  {\big\{ i\in I\ :\ {\EuScript M}_i\models\exists \alpha\in A\;\varphi(\hat ai,\alpha)\big\}\in F}\ \for for every neighborhood $A$ of $\lambda$.

\end{proposition}

\begin{proof}
  We proceed by induction on the syntax of $\varphi(x)$.
  Assume $\varphi(\hat a,\lambda)$ is atomic, say $t(\hat a,\lambda)\le s(\hat a,\lambda)$.
