...
\begin{definition}\label{def_0}
  ...
  \begin{itemize}
    ...
    \item[iii.] $M^n\to R$.
  \end{itemize}
  The interpretation of function symbolss of sort $R^n\to R$ is required to be continuous, the interpretation of function symbolss of sort $M^n\to R$ is required to be bounded, i.e.\@ the range is contained in a compact set.
...
Let $M^I$ denote the set of functions $\hat a:I\to M$.

\begin{fact}\label{fact_unifcont}
  Let $t(x;y)$ be a tuple of terms of sort $M^{|x|}\times R^{|y|}\to R$ such that 

  \ceq{\hfill\{i\in I : t^{\EuScript M}(\hat ai;\alpha)\in U\}}{\in}{F}

  for some given $\hat a\in (M^I)^{|x|}$ and $\alpha\in R^{|y|}$.
  Then there is a an open $V\ni\alpha$ such that for everu $\alpha'\in V$
...
\begin{proof}
  By induction. 
  If $t$ is a function symbol $f$ then $x$ or $y$ do not occor in $t$.
...
\begin{proof}
  ...
  Assume that $\{i:{\EuScript M}\models t(\hat ai;\alpha)\in C\}\in F$.
  Then $F\mbox{-}\lim_i t^{\EuScript M}(\hat ai;\alpha)\in C$.
  Let $D$ be a compact neighborhood of $C$.
  By Fact~\ref{fact_unifcont}, there is an open $V\ni\alpha$ such that 
  $\{i:{\EuScript M}\models t(\hat ai;\alpha')\in D\}\in F$ for every $\alpha'\in D$
  Then $\{i:{\EuScript M}\models t'(\hat ai\,;\alpha_i)\in D\}\in F$ follows.
