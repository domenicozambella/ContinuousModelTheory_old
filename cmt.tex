...
\begin{definition}\label{def_0}
  We will work with $L$-structures of the form ${\EuScript M}=\langle  M,R\rangle$, where $R$ is fixed.
  Moreover, we are given for each $n$ a Hausdorff topology on $R^n$.
  We assume that all compact sets of $R^n$ are definable in $L$.
  We require that the functions symbols of sort $M^n\times R^m\to R$  are interpreted in functions such that
  \begin{itemize}
    \item[1?] $f^{\EuScript M}(a,\mbox{-}):R^m\to R$ is continuous uniformly in $a\in M^n$, that is,
    
    \hfil$\displaystyle\bigcap_{a\in M^n}f^{\EuScript M}(a,\mbox{-})^{-1}(O)$ is open for every open $O\subseteq R$.

    \item[2?] $f^{\EuScript M}(\mbox{-},\alpha):M^n\to R$ is bounded (i.e.\@ its range is contained in a compact set) for every $\alpha\in R^m$.
  \end{itemize}
\end{definition}
...
Moreover, we require that some \textit{critical\/} function symbols only occour only inside compact relations and never under the scope of negation.

Intuitively, the function symbols are divided in three classes according to their sort
  \begin{itemize}
    \item[1.] $M^n\times R^m\to M$
    \item[2.] $M^0\times R^m\to R$
    \item[3.] $M^{n+1}\times R^m\to R$
  \end{itemize}
Functions in the latter group are the \textit{critical\/} one and will be dealt with caution.
...
\begin{definition}\label{def_1}
  ...
  \begin{itemize}
  \item[i.] $\mathds{L}$ contains all formulas without quantifiers of sort $R$ and function of sort $M^{n+1}\times R^m\to R$; 
...
\begin{fact}
  The structure ${\EuScript N}$ satisfies the conditions of Definition~\ref{def_0}.
\end{fact}

The following fact is easily proved by induction on the syntax.

\begin{fact}
  If no function symbol of sort $M^{n+1}\times R^m\to R$ occurs in   $t(y;z)$ then 
...
For $\varphi(x),\varphi'(x)\in\mathds{L}(A)$ we say that $\varphi'>\varphi$ if $\varphi'$ has been obtained replacing each atomic formulas $t\in C$ occurring in $\varphi$ by $t\in D$ for some closed neighborhood of $C$.
...
\end{proposition}
\begin{proof}
  First note that if $\varphi(x;y)$ is as in (i) of Definition~\ref{def_1} then, after absorbing $\alpha$ into le language, the proposition reduces to the classical \L\v{o}\'s Theorem.
