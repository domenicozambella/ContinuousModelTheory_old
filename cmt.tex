...
By \emph{$\mathds{T}_R\circ\mathds{T}_M$\/} we  denote the set of the terms of the form $t_R(t_M;\alpha)$, where $t_R\in\mathds{T}_R$ and $t_M$ is a tuple of terms in $\mathds{T}_M$, finally $\alpha\in R^m$.
...
If $f:I\to R$ and $\lambda\in R$ we write

\hfil$\displaystyle F\mbox{-}\lim_{i}f(i)=\lambda$

if $f^{-1}[A]\in F$ for every $A\subseteq R$ that is a neighborhood of $\lambda$.
Such a $\lambda$ is always unique (by properties of filters).
...
\begin{proof}
   The proof is clear if (as in RVL) we only allow functions of sort $M^n\times R^m\to R$ with $n\cdot m=0$.
\end{proof}
