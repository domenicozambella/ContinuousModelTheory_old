...
We call $M$ the \emph{unit ball\/} of ${\EuScript M}$: here is where the action takes place.
...
\begin{proof}
  ...
  We are confusing $( M^I)^{|x|}$ with $( M^{|x|})^I$ as it simplifies notation.
  ...
  By Lemma~\ref{}, it suffices to prove that ${\EuScript N}\models{}^{\varepsilon\kern-2pt}\varphi(\hat a)$ for every $\varepsilon>0$.
  Suppose not, then by \L\v o\'s Theorem (with $\varepsilon$ for $\lambda$), for some $\varepsilon'>0$ the set 

  \ceq{\hfill\Big\{\xi(x)\in I}{ :}{ {\EuScript M}\not\models{}^{\varepsilon+\varepsilon'\kern-2pt}\varphi(a_\xi)\Big\}}

  belongs to $F$.
  This is a contradiction because $B\subseteq F$ contains the set $X_{\varphi,\varepsilon+\varepsilon'}$. 
\end{proof}
